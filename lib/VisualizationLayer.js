  ...
  _createClass(VisualizationLayer, [{
    ...
    value: function componentDidUpdate() {
      ...
      if (this.props.canvasContext && this.canvasDrawing.length > 0) {
        this.canvasDrawing.forEach(function (piece) {
          ...
          context.setTransform(1, 0, 0, 1, 0, 0);
          (_context = context).translate.apply(_context, _toConsumableArray(adjustedPosition));
    ...
    value: function componentWillReceiveProps(np) {
      ...
      if (update === true && (!np.dataVersion || np.dataVersion && np.dataVersion !== this.state.dataVersion)) {
        ...
        Object.keys(renderPipeline).forEach(function (k) {
          ...
          if (pipe.data && _typeof(pipe.data) === "object" || pipe.data.length > 0) {
    ...
    value: function render() {
      ...
      var matte = props.matte,
          ...
          position = _props$position === undefined ? [0, 0] : _props$position;
