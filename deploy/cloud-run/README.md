...
For a tagged release, the release workflow creates a fresh deployment source
artifact after the exact npm tarball is public. The lockfile is generated by npm
and its `semiotic` integrity must equal the immutable release tarball. To prepare
the same artifact manually from a release checkout:
...
npm run prepare:cloud-run-release -- \
  --version "$VERSION" \
  --output-dir "$DEPLOY_SOURCE"
npm ci --ignore-scripts --prefix "$DEPLOY_SOURCE"
```
...
## Automated tagged-release deploy

The `Release` GitHub workflow deploys stable through the protected
`stable-mcp` environment after npm publication. Configure these environment
secrets for GitHub OIDC:

- `GCP_WORKLOAD_IDENTITY_PROVIDER`
- `GCP_RELEASE_SERVICE_ACCOUNT`

That service account must be narrowly authorized to submit source builds and
update `semiotic-mcp-server` in project `semiotic-mcp`, region `us-west1`.
Environment protection should require maintainer approval. The workflow then
runs the complete hosted smoke with the expected package version, release
commit, and build id. The MCP Registry job depends on that live verification,
so an old or failed stable revision cannot be advertised as the new release.

## Manual deploy fallback

```sh
gcloud run deploy semiotic-mcp-server --source "$DEPLOY_SOURCE" --region us-west1 \
  --allow-unauthenticated --memory 1Gi \
  --update-env-vars "MCP_ALLOWED_HOSTS=semiotic-mcp-server-481507046413.us-west1.run.app"
```
...
Use the prepared source only for the released version that is meant to be
public. Start from a clean checkout of that release tag and generate
`DEPLOY_SOURCE` with `prepare:cloud-run-release` as shown above. Never deploy
the repository root or an arbitrary `main` checkout:
...
VERSION="$(node -p "require('./package.json').version")"
DEPLOY_SOURCE="$(mktemp -d)"
npm run prepare:cloud-run-release -- --version "$VERSION" --output-dir "$DEPLOY_SOURCE"
npm ci --ignore-scripts --prefix "$DEPLOY_SOURCE"
...
node scripts/smoke-hosted-mcp.mjs \
  ...
  --expected-package-version X.Y.Z \
  --expected-sha "${SEMIOTIC_RELEASE_SHA}" \
  --expected-build-id release-vX.Y.Z
```

`git status --short` must be empty. The preparation script proves that the
wrapper dependency, npm-generated lock, and public npm integrity all identify
the released version. If deploying from a reviewed release commit instead of a
tag, substitute that full commit SHA for `vX.Y.Z` only after the same checks.
The Node one-liner normalizes the tag timestamp to the ISO UTC form required by
the hosted smoke.
...
| `MCP_MAX_CONCURRENT_UPLOADS` | `16` | Maximum bodies being received or parsed at once, independent of execution capacity. Excess uploads get 429 and the connection closes; they are not queued. Slots release after parsing or any failure, before execution admission. At the default body ceiling, active readers retain at most 64 MB of raw body chunks, plus transient parsing overhead. |
