...
const { runQualityScorecard } =
  await import("../dist/semiotic-ai.module.min.js")
const { CANONICAL_FIXTURES } = await import("../dist/semiotic-ai.module.min.js")

if (!runQualityScorecard || !CANONICAL_FIXTURES) {
  console.error(
    "❌ Scorecard helpers not found in dist/semiotic-ai.module.min.js — rebuild with `npm run dist`."
  )
  process.exit(1)
}
...
console.log(
  `Overall variant utilization: ${fmtPct(report.summary.overallVariantUtilization)}`
)
console.log("")
...
for (const f of report.perFixture) {
  const top = f.topPick
    ? `${f.topPick.component}${f.topPick.variantKey ? "/" + f.topPick.variantKey : ""} (${fmtScore(f.topPick.score)})`
    : "—"
  const agreement =
    f.expertAgreement === null ? "  " : f.expertAgreement ? "✓ " : "✗ "
  const intent = f.intent ? ` [${f.intent}]` : ""
  console.log(`  ${agreement}${f.fixture}${intent}`)
  console.log(
    `     top: ${top}, fitting=${f.fittingCount}, rejected=${f.rejectedCount}`
  )
...
console.log(
  "Weakest descriptors (sorted by expert-agreement count, ascending):"
)
const weakest = report.perCapability.slice(0, 12)
for (const c of weakest) {
  console.log(
    `  ${c.component.padEnd(28)} fits=${String(c.fitsOn).padStart(2)}  reject=${String(c.rejectedOn).padStart(2)}  top3=${String(c.inTopThreeOn).padStart(2)}  agree=${c.expertAgreementCount}  avg=${fmtScore(c.averageScore)}  caveat=${fmtPct(c.caveatCoverage)}  variant=${fmtPct(c.variantUtilization)}`
  )
}
console.log("")
