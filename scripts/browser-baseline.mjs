...
export async function runBrowserBaseline(rawArgs = process.argv.slice(2)) {
  ...
  if ([...args].some((arg) => !["--write", "--check", "--compare", "--print"].includes(arg))) {
    throw new Error("Usage: node scripts/browser-baseline.mjs --write|--check|--compare|--print")
  }
  ...
  if (write) {
    if (existsSync(BROWSER_BASELINE_PATH)) {
      ...
      const regression = compareBrowserBaselines(baseline, current, { enforceStatic: false })
      ...
      process.stdout.write("ℹ no prior browser baseline exists; recording an explicitly reviewed bootstrap snapshot\n")
    }
  ...
  const comparison = compareBrowserBaselines(baseline, current, { enforceStatic: !compare })
  ...
  if (compare) {
    process.stdout.write("✓ browser candidate has no measured regression; the recorded baseline was not changed\n")
  } else {
...
function readRecordedBaseline() {
  ...
  const errors = validateBrowserBaseline(baseline)
  if (errors.length > 0) {
...
function reportComparison(comparison, { snapshotDriftIsError }) {
  if (comparison.blockingStructuralDifferences.length > 0) {
    ...
    for (const difference of comparison.blockingStructuralDifferences) process.stderr.write("  - " + difference + "\n")
  }
  if (comparison.snapshotDifferences.length > 0) {
    const stream = snapshotDriftIsError ? process.stderr : process.stdout
    stream.write((snapshotDriftIsError ? "✗" : "ℹ") + " browser baseline static snapshot changed:\n")
