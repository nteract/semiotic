...
describe("waterfallRenderer", () => {
  ...
  it("positions bars cumulatively", () => {
    ...
    const [, y0, , h0] = (ctx.fillRect as ReturnType<typeof vi.fn>).mock.calls[0]
    ...
    const [, y1, , h1] = (ctx.fillRect as ReturnType<typeof vi.fn>).mock.calls[1]
    ...
    const [, y2, , h2] = (ctx.fillRect as ReturnType<typeof vi.fn>).mock.calls[2]
  ...
  it("draws connector lines when connectorStroke is set", () => {
    ...
    expect((ctx.save as ReturnType<typeof vi.fn>).mock.calls.length).toBe(2)
  ...
  it("handles inverted scales (arrowOfTime left)", () => {
    ...
    const [, , w, h] = (ctx.fillRect as ReturnType<typeof vi.fn>).mock.calls[0]
  ...
  it("applies bar stroke when configured", () => {
    ...
    expect((ctx.fillRect as ReturnType<typeof vi.fn>).mock.calls.length).toBe(1)
    expect((ctx.strokeRect as ReturnType<typeof vi.fn>).mock.calls.length).toBe(1)
  })
})
