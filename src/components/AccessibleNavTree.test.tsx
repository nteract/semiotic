...
describe("AccessibleNavTree — ARIA structure", () => {
  it("renders a labeled tree with leveled treeitems", () => {
    ...
    expect(items[0]).toHaveAttribute(
      "aria-label",
      expect.stringContaining("A line chart of sales by month")
    )
    // leaves carry posinset/setsize for "n of m"
    const leaf = items.find(
      (el) => el.getAttribute("aria-label") === "Feb: 250"
    )!
  ...
  it("uses theme-aware colors for visible selected rows", () => {
    ...
    expect(style).toContain(
      "background: var(--semiotic-surface, var(--semiotic-grid, var(--semiotic-bg, #f0f4f8)))"
    )
  })
  ...
  it("collapses series branches until expanded", () => {
    ...
    expect(screen.getByLabelText(/Series West:/)).toHaveAttribute(
      "aria-expanded",
      "true"
    )
  })
...
describe("AccessibleNavTree — keyboard", () => {
  ...
  it("ArrowRight expands a collapsed branch, ArrowLeft collapses it", () => {
    ...
    fireEvent.keyDown(screen.getByLabelText(/Series West:/), {
      key: "ArrowRight"
    })
    expect(screen.getByLabelText(/Series West:/)).toHaveAttribute(
      "aria-expanded",
  ...
  it("fires onActiveChange as selection moves", () => {
    ...
    render(
      <AccessibleNavTree tree={singleSeries} onActiveChange={onActiveChange} />
    )
  ...
  it("navigates hierarchy parents as spoken, expandable semantic nodes", () => {
    const hierarchy = buildNavigationTree("TreeDiagram", {
      data: {
        name: "Company",
        children: [
    ...
    render(
      <AccessibleNavTree
        tree={hierarchy}
        label="Organization hierarchy navigation"
        visible
      />
    )

    const root = screen.getByRole("treeitem", {
      name: /A tree chart with 2 leaves and 3 total descendants/
    })
    root.focus()
    fireEvent.keyDown(root, { key: "ArrowRight" })
    const company = screen.getByRole("treeitem", {
      name: "Company: 2 direct children, 3 total descendants, 2 leaves, leaf total 6."
    })
    ...
    fireEvent.keyDown(company, { key: "ArrowRight" })
    const engineering = screen.getByRole("treeitem", {
      name: "Engineering: 1 direct child, 1 total descendant, 1 leaf, leaf total 4."
    })
    expect(engineering).toHaveFocus()
    expect(engineering).toHaveAttribute("aria-level", "3")
    expect(engineering).toHaveAttribute("aria-posinset", "1")
    expect(engineering).toHaveAttribute("aria-setsize", "2")
  })

  it("announces choropleth metric groups before descending to ranked regions", () => {
    const geo = buildNavigationTree("ChoroplethMap", {
      areas: [
    ...
    render(
      <AccessibleNavTree
        tree={geo}
        label="Regional score navigation"
    ...
    const root = screen.getByRole("treeitem", {
      name: /Values are available for 4 of 4 regions/
    })
    root.focus()
    fireEvent.keyDown(root, { key: "ArrowDown" })
    fireEvent.keyDown(
      screen.getByRole("treeitem", { name: /Highest values:/ }),
      {
        key: "ArrowDown"
      }
    )
    const middle = screen.getByRole("treeitem", {
      name: "Middle values: 2 regions, range 40 to 60, average 50, total 100."
    })
    expect(middle).toHaveFocus()

    fireEvent.keyDown(middle, { key: "ArrowRight" })
    fireEvent.keyDown(middle, { key: "ArrowRight" })
    const beta = screen.getByRole("treeitem", {
      name: "Beta: 60, rank 2 of 4."
    })
    expect(beta).toHaveFocus()
    expect(beta).toHaveAttribute("aria-level", "3")
    expect(beta).toHaveAttribute("aria-posinset", "1")
    expect(beta).toHaveAttribute("aria-setsize", "2")
    expect(onObservation).toHaveBeenLastCalledWith(
      expect.objectContaining({
        type: "focus",
        datum: expect.objectContaining({
          properties: { name: "Beta", score: 60 }
        }),
...
describe("AccessibleNavTree — semantic observations", () => {
  ...
  it("activates an annotation leaf by stable id with Enter", () => {
    ...
    const annotation = screen.getByLabelText(
      /^A widget labeled "Secret console"\.$/
    )
...
describe("AccessibleNavTree — reception telemetry", () => {
  ...
  it("records nav-node-focused on keyboard traversal when the arc is enabled", () => {
    ...
    fireEvent.keyDown(screen.getAllByRole("treeitem")[0], { key: "ArrowDown" })
    const focus = getConversationArcStore()
      .getEvents()
      .filter((e) => e.type === "nav-node-focused")
  ...
  it("records nav-branch-expanded on expand (and the expanded flag on collapse)", () => {
    ...
    const toggles = getConversationArcStore()
      .getEvents()
      .filter((e) => e.type === "nav-branch-expanded")
