import { packBandsBySilhouette, placeWithMinGap, type SilhouetteSample } from "../../../recipes/layout1d"
import type {
...
export function slotStableId(slot: ProcessSankeySlot): string {
  return slot.occupants.map((d) => d.id).sort(compareProcessSankeyIds).join("\u0000")
}
...
export function processSankeyGapPaddings(
  ...
  return slots.slice(0, -1).map((slot, index) => {
    ...
    return slot.group != null && slot.group === next.group ? groupPadding : padding
  })
}
...
export function computeSlotPairClearance(
  ...
  return packBandsBySilhouette(slotSilhouettes([upper, lower], nodeData))
...
function stackCenters(
  ...
  const gapPaddings = processSankeyGapPaddings(
    slots, padding, options.groupPadding ?? 0,
  )
  const centers = [padding + slots[0].peak.topPeak * spacingScale]
  for (let i = 1; i < slots.length; i++) {
    centers.push(centers[i - 1] + adjacentClearance[i - 1] * spacingScale + gapPaddings[i - 1])
  }
  const bottom = centers[centers.length - 1] + slots[slots.length - 1].peak.botPeak * spacingScale + padding
...
function attachmentOffset(attachment: ProcessSankeyAttachment, valueScale: number): number {
  ...
  if (attachment.kind === "out") {
    ...
    return boundaryOffset + (attachment.side === "top" ? -before + value / 2 : before - value / 2)
  }
  const after = attachment.sideMassAfter * valueScale
  return boundaryOffset + (attachment.side === "top" ? -after + value / 2 : after - value / 2)
}
...
function hugCenters(
  ...
  const minGaps = adjacentClearance.map((clearance, index) =>
    clearance * valueScale + gapPaddings[index],
  )
  ...
  for (let iteration = 0; iteration < 12; iteration++) {
    ...
    for (const edge of edges) {
      ...
      desiredSum[sourceSlot] += (centers[targetSlot] + targetOffset - sourceOffset) * weight
      desiredWeight[sourceSlot] += weight
      desiredSum[targetSlot] += (centers[sourceSlot] + sourceOffset - targetOffset) * weight
    ...
    let movement = 0
    for (let i = 0; i < n; i++) movement = Math.max(movement, Math.abs(next[i] - centers[i]))
    centers = next
...
export function computeSlotGeometry(
  ...
  let slotCenter = options.lanePlacement === "hug"
    ? hugCenters(slots, slotByNode, edges, nodeData, adjacentClearance, stackCenter, options)
    : [...stackCenter]
  if (options.lanePlacement === "hug") {
    const centersFor = (positions: readonly number[]): Record<string, number> => {
    ...
    if (totalPixelEdgeLength(centersFor(slotCenter), edges) >
        totalPixelEdgeLength(centersFor(stackCenter), edges) + 1e-6) {
      slotCenter = [...stackCenter]
    }
  }
  const centerlines: Record<string, number> = {}
  for (const node of nodes) centerlines[node.id] = slotCenter[slotByNode[node.id]]
  ...
  const verticalUtilization = Number.isFinite(top) && Number.isFinite(bottom) && options.plotH > 0
    ? Math.max(0, Math.min(1, (bottom - top) / options.plotH))
    : 0
  return { adjacentClearance, centerlines, slotCenter, stackCenter, verticalUtilization }
}
...
function massAt(data: ProcessSankeyNodeData | undefined, time: number): { top: number; bot: number } {
  const samples = data?.samples
  if (!samples || samples.length === 0 || time < samples[0].t) return { top: 0, bot: 0 }
  let lo = 0
  let hi = samples.length - 1
  let found = 0
  while (lo <= hi) {
    const mid = (lo + hi) >>> 1
    if (samples[mid].t <= time) { found = mid; lo = mid + 1 }
    else hi = mid - 1
  }
...
function cubic(value0: number, value1: number, value2: number, value3: number, t: number): number {
  const mt = 1 - t
  return mt * mt * mt * value0 + 3 * mt * mt * t * value1 +
    3 * mt * t * t * value2 + t * t * t * value3
}
...
export function measureTransitOcclusion(
  ...
  const control = options.ribbonLane === "source" ? 0.85 : options.ribbonLane === "target" ? 0.15 : 0.5
  let total = 0
  for (const edge of edges) {
    const sourceSlot = slotByNode[edge.source]
    const targetSlot = slotByNode[edge.target]
    if (sourceSlot == null || targetSlot == null || Math.abs(sourceSlot - targetSlot) < 2) continue
    const sourceAttachment = nodeData[edge.source]?.localAttachments.get(edge.id)
    const targetAttachment = nodeData[edge.target]?.localAttachments.get(edge.id)
    if (!sourceAttachment || !targetAttachment) continue
    ...
    const [sourceTop, sourceBottom] = attachmentRange(sourceAttachment, centerlines[edge.source], valueScale)
    const [targetTop, targetBottom] = attachmentRange(targetAttachment, centerlines[edge.target], valueScale)
    ...
    for (let slotIndex = firstSlot; slotIndex <= lastSlot; slotIndex++) {
      ...
      const ratioAt = (time: number): number => {
        ...
        const ribbonTop = cubic(sourceTop, sourceTop, targetTop, targetTop, parameter)
        const ribbonBottom = cubic(sourceBottom, sourceBottom, targetBottom, targetBottom, parameter)
        let ratio = 0
        for (const occupant of slot.occupants) {
          if (occupant.id === edge.source || occupant.id === edge.target) continue
          ...
          ratio = Math.max(ratio, overlapRatio(
            ribbonTop,
            ribbonBottom,
...
export function computeLayoutQuality(
  ...
  return {
    ...
    transitOcclusion: measureTransitOcclusion(
      edges, nodeData, slots, slotByNode, centerlines, laneLifetime,
