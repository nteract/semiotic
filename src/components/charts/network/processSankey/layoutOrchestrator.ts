...
export function computeLaneLayout(
  ...
  const topPeak = createProcessSankeyRecord<number>()
  const botPeak = createProcessSankeyRecord<number>()
  ...
  const laneLifetime = createProcessSankeyRecord<ProcessSankeyLaneLifetime>()
  for (const n of nodes) {
    ...
    let tStart: number =
      explicitStart != null && Number.isFinite(explicitStart)
        ? explicitStart
        : Infinity
    let tEnd: number =
      explicitEnd != null && Number.isFinite(explicitEnd)
        ? explicitEnd
        : -Infinity
  ...
  const geometry = computeSlotGeometry(
    ...
    slotByNode,
    {
      ...
      padding,
      valueScale,
      stackValueScale: opts.stackValueScale,
      lanePlacement,
      groupPadding
    }
  )

  const effectiveSlotsHeight =
    slots.length === 0
      ? 0
...
export function computeProcessSankeyLayout(
  ...
  const groupPadding = Number.isFinite(rawGroupPadding)
    ? Math.max(0, rawGroupPadding)
    : 0

  const edgeIndex = buildEdgeIndex(nodes, edges)
  const sides = assignSides(nodes, edges, edgeIndex, pairing)
  let nodeData = createProcessSankeyRecord<ProcessSankeyNodeData>()
  for (const n of nodes) {
  ...
  nodeData = createProcessSankeyRecord<ProcessSankeyNodeData>()
  for (const n of nodes) {
    nodeData[n.id] = computeNode(n, edgeIndex, sides, undefined, nodeSizing)
  }
  ...
  const slotsHeight =
    dry.effectiveSlotsHeight ??
    dry.slots.reduce((s, slot) => s + slot.peak.topPeak + slot.peak.botPeak, 0)
  const bondedGapCount = dry.slots
    .slice(0, -1)
    .reduce(
      (count, slot, index) =>
        count +
        (slot.group != null && slot.group === dry.slots[index + 1].group
          ? 1
          : 0),
      0
    )
  ...
  const totalGaps =
    padding * 2 +
    processSankeyGapPaddings(dry.slots, padding, groupPadding).reduce(
      (sum, gap) => sum + gap,
      0
    )
  const naturalValueScale =
    slotsHeight > 0 ? Math.max(0, (plotH - totalGaps) / slotsHeight) : 1
  const valueScale =
    Number.isFinite(maxValueScale) && maxValueScale! > 0
  ...
  const maxScaleBinds =
    Number.isFinite(maxValueScale) &&
    maxValueScale! > 0 &&
    maxValueScale! + 1e-12 < naturalValueScale
  const needsGeometryRefine =
    laneOrder !== "insertion" && (lanePlacement === "hug" || maxScaleBinds)
  ...
  const initialRank = new Map(
    dry1.initialSlotOrder.map((id, index) => [id, index])
  )
  const baselineSlots = [...layout.slots].sort(
    (a, b) =>
      (initialRank.get(slotStableId(a)) ?? Infinity) -
        (initialRank.get(slotStableId(b)) ?? Infinity) ||
      compareProcessSankeyIds(slotStableId(a), slotStableId(b))
  )
  const baselineSlotByNode: SlotByNode = createProcessSankeyRecord<number>()
  baselineSlots.forEach((slot, index) => {
    for (const occupant of slot.occupants)
      baselineSlotByNode[occupant.id] = index
  })
  ...
  const layoutQualityBefore = computeLayoutQuality(
    ...
    layout.laneLifetime,
    valueScale,
    baselineGeometry.verticalUtilization,
    ribbonLane,
    domain
  )
  ...
  return {
    ...
    crossingsBefore:
      laneOrder === "insertion" ? null : layoutQualityBefore.crossings,
    crossingsAfter: laneOrder === "insertion" ? null : layoutQuality.crossings,
    lengthBefore:
      laneOrder === "insertion" ? null : layoutQualityBefore.weightedLength,
    lengthAfter:
      laneOrder === "insertion" ? null : layoutQuality.weightedLength,
    layoutQualityBefore,
    layoutQuality
  }
}
