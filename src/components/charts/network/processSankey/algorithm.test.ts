...
describe("validateProcessSankey", () => {
  ...
  it("flags missing source nodes", () => {
    ...
    expect(
      issues.some((i) => i.kind === "missing-node" && i.endpoint === "source")
    ).toBe(true)
  })
  ...
  it("flags missing target nodes", () => {
    ...
    expect(
      issues.some((i) => i.kind === "missing-node" && i.endpoint === "target")
    ).toBe(true)
  })
  ...
  it("formats each known issue kind and falls back to the raw kind", () => {
    expect(
      formatProcessSankeyIssue({ kind: "invalid-node-time", id: "n" })
    ).toContain("node n")
    expect(
      formatProcessSankeyIssue({ kind: "invalid-edge-time", id: "e" })
    ).toContain("edge e")
    expect(formatProcessSankeyIssue({ kind: "invalid-domain" })).toContain(
      "time domain"
    )
    expect(
      formatProcessSankeyIssue({ kind: "invalid-value", id: "e" })
    ...
    expect(formatProcessSankeyIssue({ kind: "unknown-kind" })).toBe(
      "unknown-kind"
    )
  })
...
describe("side assignment and node mass walks", () => {
  ...
  it("synthesizes creates and transfers so outs never make a side negative", () => {
    ...
    expect(data.localAttachments.get("ab")).toMatchObject({
      kind: "out",
      side: "top",
      value: 5
    })
    expect(data.localAttachments.get("ac")).toMatchObject({
      kind: "out",
      side: "bot",
      value: 3
    })
    expect(data.samples[0].t).toBe(-1)
    expect(data.samples.every((s) => s.topMass >= 0 && s.botMass >= 0)).toBe(
      true
    )
  })
  ...
  it("holds an instantaneous same-stage peak across an authored extent when max sizing is requested", () => {
    ...
    expect(
      Math.max(
        ...temporal.samples
          .filter((sample) => sample.t < 2)
          .map((sample) => sample.topMass + sample.botMass)
      )
    ).toBe(35)
  ...
  it("anchors synthesized side transfers at departure without creating a floating band island", () => {
    ...
    expect([...sampleTimes].every((time) => [10, 20, 30].includes(time))).toBe(
      true
    )
    ...
    expect(
      data.samples.every((sample) => sample.topMass >= 0 && sample.botMass >= 0)
    ).toBe(true)
  })
  ...
  it("assigns same-row handoffs to the side that holds their visible mass", () => {
    ...
    const sides = new Map([
      ...
      [
        "eng-release-8",
        { sourceSide: "bot" as const, targetSide: "bot" as const }
      ],
      [
        "eng-release-5",
        { sourceSide: "bot" as const, targetSide: "bot" as const }
    ...
    expect(sides.get("eng-release-8")).toMatchObject({
      sourceSide: "top",
      targetSide: "top"
    })
    expect(sides.get("eng-release-5")).toMatchObject({
      sourceSide: "bot",
      targetSide: "bot"
    })
  })
...
describe("band geometry helpers", () => {
  ...
  it("returns the correct y range for every attachment side/kind", () => {
    ...
    expect(
      attachmentYRange(
        { ...base, kind: "out", side: "top" } as ProcessSankeyAttachment,
        100,
        10
      )
    ).toEqual([60, 80])
    expect(
      attachmentYRange(
        { ...base, kind: "out", side: "bot" } as ProcessSankeyAttachment,
        100,
        10
      )
    ).toEqual([120, 140])
    expect(
      attachmentYRange(
        { ...base, kind: "in", side: "top" } as ProcessSankeyAttachment,
        100,
        10
      )
    ).toEqual([40, 60])
    expect(
      attachmentYRange(
        { ...base, kind: "in", side: "bot" } as ProcessSankeyAttachment,
        100,
        10
      )
    ).toEqual([140, 160])
  ...
  it("builds system-in and system-out gradient cutouts clipped to the band", () => {
    ...
    const sourceStubs = buildBandCutoutsForNode(
      "A",
      edges,
      layout,
      xScale,
    ...
    const targetStubs = buildBandCutoutsForNode(
      "B",
      edges,
      layout,
      xScale,
    ...
    expect(
      buildBandCutoutsForNode("missing", edges, layout, xScale, [0, 40])
    ).toEqual([])
  })
...
describe("computeProcessSankeyLayout", () => {
  ...
  it("places a multi-slot bonded feeder block in line with its exclusive ungrouped sink", () => {
    ...
    const foundingSlots = ["Red", "White", "Blue"].map(
      (id) => layout.slotByNode[id]
    )
  ...
  it("keeps a shared node group contiguous and removes its internal gutters", () => {
    ...
    const edges: ProcessSankeyEdge[] = [
      {
        id: "states",
        source: "A-states-before",
        target: "A-states-after",
        value: 3,
        startTime: 2,
        endTime: 8
      },
      {
        id: "foreign-1",
        source: "B-foreign-before",
        target: "B-foreign-after",
        value: 2,
        startTime: 2,
        endTime: 8
      },
      {
        id: "territories",
        source: "C-territories-before",
        target: "C-territories-after",
        value: 2,
        startTime: 2,
        endTime: 8
      },
      {
        id: "foreign-2",
        source: "D-foreign-before",
        target: "D-foreign-after",
        value: 2,
        startTime: 2,
        endTime: 8
      },
      {
        id: "occupied",
        source: "E-occupied-before",
        target: "E-occupied-after",
        value: 1,
        startTime: 2,
        endTime: 8
      }
    ...
    const groupedSlots = [
      ...new Set(
        nodes
          ...
          .map((node) => layout.slotByNode[node.id])
      )
    ].sort((a, b) => a - b)
    ...
    for (let index = 1; index < groupedSlots.length; index += 1) {
      const upper = layout.slots[groupedSlots[index - 1]].occupants.find(
        (occupant) => occupant.id.endsWith("before")
      )!
      const lower = layout.slots[groupedSlots[index]].occupants.find(
        (occupant) => occupant.id.endsWith("before")
      )!
      const upperData = layout.nodeData[upper.id]
      const lowerData = layout.nodeData[lower.id]
      const upperBottom =
        layout.centerlines[upper.id] + upperData.botPeak * layout.valueScale
      const lowerTop =
        layout.centerlines[lower.id] - lowerData.topPeak * layout.valueScale
    ...
    const activeRanges = nodes
      .filter(
        (node) => node.group === "united-states" && node.id.endsWith("before")
      )
      .map((node) => {
    ...
    for (let index = 1; index < activeRanges.length; index += 1) {
      expect(activeRanges[index][0] - activeRanges[index - 1][1]).toBeCloseTo(
        4,
        6
      )
    }
  ...
  it("uses the resolved far-end order to break equal-time attachment ties", () => {
    ...
    for (const edge of sameStart) {
      expect(midpoint(departureLayout, edge, "source")).toBeCloseTo(
        midpoint(departureLayout, edge, "target")
      )
    }
    ...
    for (const edge of sameEnd) {
      expect(midpoint(arrivalLayout, edge, "source")).toBeCloseTo(
        midpoint(arrivalLayout, edge, "target")
      )
    }
  })
  ...
  it("emits crossings/length quality metrics", () => {
    ...
    const layout = computeProcessSankeyLayout(nodes, edges, opts)
    expect(
      typeof layout.crossingsBefore === "number" ||
        layout.crossingsBefore === null
    ).toBe(true)
    expect(
      typeof layout.crossingsAfter === "number" ||
        layout.crossingsAfter === null
    ).toBe(true)
    expect(
      typeof layout.lengthBefore === "number" || layout.lengthBefore === null
    ).toBe(true)
    expect(
      typeof layout.lengthAfter === "number" || layout.lengthAfter === null
    ).toBe(true)
  })
  ...
  it("extends lane lifetime + target band when systemOutTime > endTime", () => {
    ...
    expect(
      Math.max(...hinted.nodeData.B.samples.map((s) => s.t))
    ).toBeGreaterThanOrEqual(7)
    // Mass profile within the natural range is unchanged.
    const bPlainInside = plain.nodeData.B.samples.filter(
      (s) => s.t >= 3 && s.t <= 5
    )
    const bHintedInside = hinted.nodeData.B.samples.filter(
      (s) => s.t >= 3 && s.t <= 5
    )
