...
import { compareProcessSankeyIds } from "./layoutGeometry"
...
function pairKey(a: string, b: string): string {
  return compareProcessSankeyIds(a, b) <= 0
    ? `${a}\u0000${b}`
    : `${b}\u0000${a}`
}
...
function continuityDegreeMaps(edges: readonly ProcessSankeyEdge[]): {
  ...
  for (const edge of edges) {
    if (!outgoingPartners.has(edge.source))
      outgoingPartners.set(edge.source, new Set())
    if (!incomingPartners.has(edge.target))
      incomingPartners.set(edge.target, new Set())
    outgoingPartners.get(edge.source)!.add(edge.target)
    incomingPartners.get(edge.target)!.add(edge.source)
  }
  return {
    incoming: new Map(
      [...incomingPartners].map(([id, partners]) => [id, partners.size])
    ),
    outgoing: new Map(
      [...outgoingPartners].map(([id, partners]) => [id, partners.size])
    )
  }
}
...
function mixesExclusiveRoles(
  ...
  return members.some(
    (other) => !exclusiveRolesCompatible(id, other, exclusive)
  )
}
...
function buildPackingConflicts(
  ...
  for (const members of successors.values())
    addMemberPairs([...members], sharedPredecessorPairs)
  for (const members of predecessors.values())
    addMemberPairs([...members], sharedSuccessorPairs)
...
function chronologicalColoring(
  ...
  exclusive: ReadonlyMap<
    string,
    { partner: string; weight: number }
  > = new Map()
  ...
  for (const id of ids) {
    ...
    let slot = slotMembers.findIndex(
      (members) =>
...
function improveLargeContinuityColoring(
  ...
  for (const edge of edges) {
    ...
    flowAffinity[source].set(
      target,
      (flowAffinity[source].get(target) ?? 0) + weight
    )
    flowAffinity[target].set(
      source,
      (flowAffinity[target].get(source) ?? 0) + weight
    )
  }
  ...
  for (
    let first = 0;
    first < ids.length - 1 && rolePairs < LARGE_PACKING_ROLE_PAIR_MAX;
    first++
  ) {
    ...
    for (let second = first + 1; second < ids.length; second++) {
      ...
      rolePairs++
      const weight =
        Math.min(roleWeights[first], roleWeights[second]) ||
      ...
      const relation = exclusiveRolesCompatible(
        ids[first],
        ids[second],
        exclusive
      )
  ...
  const improves = (flow: number, role: number): boolean =>
    flow > MASS_EPSILON ||
    (Math.abs(flow) <= MASS_EPSILON && role > MASS_EPSILON)
  ...
  const beats = (
    ...
    (Math.abs(flow - bestFlow) <= MASS_EPSILON &&
      role > bestRole + MASS_EPSILON)

  let evaluations = 0
  for (
    let pass = 0;
    pass < 32 && evaluations < LARGE_PACKING_SWAP_EVALUATION_MAX;
    pass++
  ) {
    ...
    for (let node = 0; node < ids.length; node++) {
      for (let targetSlot = 0; targetSlot < members.length; targetSlot++) {
        ...
        if (
          !members[targetSlot].every(
            (other) => !packingConflicts.conflicts(ids[node], ids[other])
          )
        )
          continue
    ...
    for (let first = 0; first < ids.length - 1; first++) {
      for (let second = first + 1; second < ids.length; second++) {
        ...
        if (
          !members[secondSlot].every(
            (other) =>
              other === second ||
              !packingConflicts.conflicts(ids[first], ids[other])
          )
        )
          continue
        if (
          !members[firstSlot].every(
            (other) =>
              other === first ||
              !packingConflicts.conflicts(ids[second], ids[other])
          )
        )
          continue
    ...
    members[firstSlot].splice(
      members[firstSlot].indexOf(bestFirst),
      1,
      bestSecond
    )
    members[secondSlot].splice(
      members[secondSlot].indexOf(bestSecond),
      1,
      bestFirst
    )
...
function continuityColoring(
  ...
  if (
    ids.length === 0 ||
    ids.length > EXACT_PACKING_NODE_MAX ||
    baseline.slotCount <= 1
  ) {
    return baseline.assignment
  }
  ...
  for (let i = 0; i < ids.length; i++) {
    for (let j = i + 1; j < ids.length; j++) {
      ...
      if (exclusive.has(ids[i]) && exclusive.has(ids[j])) {
        const roleWeight =
          Math.min(roleWeights[i], roleWeights[j]) ||
  ...
  const search = (
    ...
    if (index === ids.length) {
      evaluations++
      if (
        currentFlow > bestScore.flow ||
        (currentFlow === bestScore.flow &&
          currentAffinity > bestScore.affinity) ||
        (currentFlow === bestScore.flow &&
          currentAffinity === bestScore.affinity &&
    ...
    for (let slot = 0; slot <= lastCandidate; slot++) {
      if (
        members[slot].some((other) =>
          packingConflicts.conflicts(ids[index], ids[other])
        )
      ) {
        continue
      }
