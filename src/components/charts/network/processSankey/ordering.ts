...
function mapForOrder(order: readonly ProcessSankeySlot[]): SlotByNode {
...
function bondedSlotUnits(order: readonly ProcessSankeySlot[]): BondedSlotUnit[] {
...
function bondedSlotOrder(order: readonly ProcessSankeySlot[]): ProcessSankeySlot[] {
  if (!order.some((slot) => slot.group)) return [...order]
  return bondedSlotUnits(order).flatMap((unit) => unit.slots)
}

function hasMultiSlotBond(order: readonly ProcessSankeySlot[]): boolean {
...
function flattenBondedUnits(units: readonly BondedSlotUnit[]): ProcessSankeySlot[] {
 ...
 */
function unitRelationsFromSlots(
 ...
 */
function orderWithinBondedUnits(
  units: readonly BondedSlotUnit[],
  slotRelations: readonly WeightedOrderRelation<ProcessSankeySlot>[],
