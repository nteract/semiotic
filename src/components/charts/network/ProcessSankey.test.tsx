...
vi.mock("../../stream/StreamNetworkFrame", () => {
  return {
    ...
    >((props, _ref) => {
      ...
      return (
        <div className="stream-network-frame" data-testid="frame">
          <svg />
        </div>
      )
    })
...
describe("ProcessSankey HOC", () => {
  ...
  it("forwards bands + ribbons through layoutConfig to the frame", () => {
    ...
    if (!lastFrameProps)
      throw new Error("Expected StreamNetworkFrame to render")
    ...
    const alice = lastFrameProps.layoutConfig.bands.find(
      (band) => band.id === "Alice"
    )!
  ...
  it("uses an explicit category map for both node bands and source ribbons", () => {
    ...
    expect(lastFrameProps?.layoutConfig.bands.map((band) => band.fill)).toEqual(
      ["#ff0000", "#0000ff"]
    )
    expect(
      lastFrameProps?.layoutConfig.ribbons.map((ribbon) => ribbon.fill)
    ).toEqual(["#ff0000"])
  })
  ...
  it.each([
    ...
    "grows a small explicit %s margin for its chart-owned legend",
    (position, minimum) => {
      render(
        <TooltipProvider>
          <ProcessSankey
  ...
  it("uses frameProps legendPosition while reserving its chart-owned legend", () => {
    ...
    expect(
      (lastFrameProps as unknown as Record<string, unknown>)
        ?.__legendMarginReservedFor
    ).toBe(lastFrameProps?.legend)
  })

  it("measures long chart-owned legend labels beyond its compact side minimum", () => {
    render(
      <TooltipProvider>
        <ProcessSankey
          nodes={[
            {
              id: "Alice",
              category: "A category label that needs a wider legend"
            },
            {
              id: "Eng",
              category: "Another category label that needs a wider legend"
            }
          ]}
          edges={sampleEdges}
          domain={DOMAIN}
    ...
    expect(lastFrameProps?.margin.right).toBeGreaterThan(140)
    expect(
      (lastFrameProps as unknown as Record<string, unknown>)
        ?.__legendMarginReservedFor
    ).toBe(lastFrameProps?.legend)
  ...
  it("forwards a named selection into the custom Sankey scene", () => {
    function LensWriter() {
      const lens = useSelection({
        name: "process-sankey-lens",
        fields: ["claimLens"]
      })
      return (
        <button
    ...
    expect(
      lastFrameProps?.layoutSelection?.predicate({ claimLens: "economic" })
    ).toBe(true)
    expect(
      lastFrameProps?.layoutSelection?.predicate({ claimLens: "cultural" })
    ).toBe(false)
  })
  ...
  it("projects time top-to-bottom in vertical orientation", () => {
    ...
    expect(
      lastFrameProps?.layoutConfig.bands.every(
        (item) => item.labelAnchor === "middle"
      )
    ).toBe(true)
  })
  ...
  it("borrows proven feeder runway without changing authored event timing", () => {
    ...
    expect(smoothedRibbon?.rawDatum).toMatchObject({
      startTime: 96,
      endTime: 100
    })
    const feederBand = lastFrameProps?.layoutConfig.bands.find(
      (band) => band.id === "Feeder"
    )
    expect(maximumPathY(feederBand?.pathD ?? "")).toBe(smoothedPoints?.[0].y)
    const renderedEdges = lastFrameProps?.edges
    expect(
      Array.isArray(renderedEdges) ? renderedEdges[0]?.bezier : undefined
  ...
  it("resolves groupBy and emits same-group ribbons as one contiguous block", () => {
    const nodes = ["A", "B", "C", "D", "E"].flatMap((prefix) => [
      {
        id: `${prefix}-before`,
        bond: ["A", "C", "E"].includes(prefix) ? "united-states" : undefined,
        xExtent: [0, 2]
      },
      {
        id: `${prefix}-after`,
    ...
    const starts = new Map(
      lastFrameProps?.layoutConfig.ribbons.map((ribbon) => [
        ribbon.id,
        ribbon.bezier?.points[0].y ?? NaN
      ])
    )
    const grouped = [starts.get("A")!, starts.get("C")!, starts.get("E")!].sort(
      (a, b) => a - b
    )
    for (const id of ["B", "D"]) {
  ...
  it("update() returns the previous records synchronously", () => {
    ...
    function Harness() {
      ...
      useEffect(() => {
        ...
        capturedPrevious = ref.current.update("e1", (e) => ({
          ...e,
          value: 99
        }))
      }, [])
