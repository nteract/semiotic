...
describe("getColor", () => {
  ...
  it("returns color from a string field with a color scale", () => {
    const scale = (v: string) => (v === "A" ? "#aaa" : "#bbb")
  ...
  it("maps function colorBy result through colorScale when value is a category name", () => {
    ...
    const scale = (v: string) => v === "Short" ? "#0f0" : "#f00"
  ...
  it("passes through CSS hex colors from function colorBy without colorScale mapping", () => {
    ...
    const scale = vi.fn((_v: string) => "#999")
  ...
  it("returns function result directly when no colorScale is provided", () => {
    const colorFn = (_d: Datum) => "CategoryA"
  ...
  it("maps non-color strings through colorScale even when they resemble color names", () => {
    ...
    const scale = vi.fn((_v: string) => "#123456")
