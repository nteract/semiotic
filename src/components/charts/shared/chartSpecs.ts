...
export const CHART_SPECS: Record<string, ChartSpec> = {
  ...
  BigNumber: {
    ...
    ownProps: {
      ...
      headerSlot: { type: ["object", "function"], omitFromSchema: true, description: "Replace the entire header (label + caption) slot" },
      valueSlot: { type: ["object", "function"], omitFromSchema: true, description: "Replace the focal value slot" },
      deltaSlot: { type: ["object", "function"], omitFromSchema: true, description: "Replace the delta / comparison / target row" },
      trendSlot: { type: ["object", "function"], omitFromSchema: true, description: "Wide / rectangular chart embedded beneath the value — e.g. a LineChart / AreaChart in mode=\"sparkline\". ReactNode or (ctx) => ReactNode; the function form receives the resolved level / color / sentiment / pushBuffer." },
      footerSlot: { type: ["object", "function"], omitFromSchema: true, description: "Free-form footer below the trend" },
      onClick: { type: "function", omitFromSchema: true },
