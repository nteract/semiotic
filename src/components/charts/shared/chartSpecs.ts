...
export const CHART_SPECS: Record<string, ChartSpec> = {
  ...
  NetworkHOPsChart: {
    ...
    category: "physics",
    description: "Network hypothetical outcome plot. Animates sampled realizations of uncertain or probabilistic graphs while node positions stay anchored to the aggregate topology.",
    required: [],
    dataShape: "network",
    dataAccessors: ["nodeIdAccessor", "nodeIDAccessor", "sourceAccessor", "targetAccessor", "edgeProbabilityAccessor"],
    propBags: ["common"],
    ownProps: {
      size: { type: "array", description: "[width, height] in pixels" },
      nodes: { type: "array", description: "Array of node objects. Optional when samples include nodes or edges can infer endpoint ids." },
      edges: { type: "array", description: "Probabilistic edge list. Each edge should include source, target, and a probability field such as p." },
      samples: { type: "array", description: "Explicit graph realizations: [{ id?, label?, nodes?, edges }]. Used instead of probabilistic edges when available." },
      nodeIdAccessor: { type: ["string", "function"], default: "id", description: "Key for node unique identifier." },
      nodeIDAccessor: { type: ["string", "function"], default: "id", description: "Legacy alias for nodeIdAccessor." },
      sourceAccessor: { type: ["string", "function"], default: "source", description: "Key for edge source node ID." },
      targetAccessor: { type: ["string", "function"], default: "target", description: "Key for edge target node ID." },
      edgeProbabilityAccessor: { type: ["string", "function"], default: "p", description: "Probability field used when edges are probabilistic rather than explicit samples." },
      sampleIndex: { type: "number", description: "Controlled realization index. Omit to animate through samples." },
      sampleRate: { type: "number", default: 1, description: "Realizations per second when sampleIndex is uncontrolled." },
      paused: { type: "boolean", default: false, description: "Pause animated sample cycling." },
      seed: { type: "number", default: 1, description: "Deterministic seed for probabilistic edge sampling." },
      anchoringStrength: { type: "number", default: 0.14, description: "Force-directed link strength used for the stable aggregate layout." },
      showAggregate: { type: "boolean", default: true, description: "Draw inactive aggregate edges faintly behind the active sample." },
      showSampleReadout: { type: "boolean", default: true, description: "Show active-edge and aggregate summary overlay." },
      activeEdgeColor: { type: "string", description: "Stroke color for edges present in the active realization." },
      activeEdgeOpacity: { type: "number", default: 0.88 },
      aggregateEdgeColor: { type: "string", default: "#94a3b8" },
      aggregateEdgeOpacity: { type: "number", default: 0.18 },
      aggregateEdgeWidth: { type: "number", default: 1 },
      edgeWidth: { type: ["number", "string", "function"], default: 2, description: "Width for active sampled edges." },
      edgeColor: { type: "string", default: "#2563eb" },
      iterations: { type: "number", default: 300, description: "Force simulation iterations for aggregate node layout." },
      nodeLabel: { type: ["string", "function"], description: "Key or accessor for node labels." },
      nodeSize: { type: ["number", "string", "function"], default: 8 },
      nodeSizeRange: { type: "array", default: [5, 20] },
      showLabels: { type: "boolean", default: false },
      layoutExecution: { type: "string", enum: ["auto", "worker", "sync"] as const, default: "auto" },
    },
    capabilities: {
      renderModes: ["hybrid"],
      supportsLegend: true,
      ...
      colorModel: "categorical",
      layoutMode: "plugin",
      specialFeatures: ["force-simulation", "uncertainty", "hypothetical-outcomes", "sample-animation"],
    },
  },
