...
export function buildEventDropPhysics<TDatum extends Datum>(
  ...
  const maxTime = times.length ? Math.max(...times) : minTime + windows.size
  ...
  data.forEach((datum, index) => {
    ...
    spawns.push({
      ...
      x: late ? area.plot.x + area.plot.width - ballRadius * 2 : xScale(windowIndex + 0.5),
      y: area.plot.y + ballRadius + 2,
      vx: late ? -18 : ((index % 3) - 1) * 8,
      vy: 0,
      mass: 1,
      spawnAt: arrivalTime,
      shape: { type: "circle", radius: ballRadius },
  ...
  const colliders = [
    ...
    ...collidersFromXScaleBins({
      ...
      yTop: area.plot.y,
      yBottom,
      closedBefore: Math.max(0, Math.floor((watermarkValue - windowStart) / windows.size)),
      lidY: area.plot.y + area.plot.height * 0.32,
      wallThickness: 6
    })
  ]
 ...
 * board's domain (window layout + current watermark) so a pushed arrival lands
 * at its true event-time x — in its window, or bounced to the late gutter —
 * instead of the center of a one-event mini-domain. Returns null when the datum
 * carries no finite event time. Omits `spawnAt` so the store drops it now.
 */
export function placeEventDropSpawn<TDatum extends Datum>(
  ...
  const { windowStart, windowCount, windowSize, watermarkValue, plot } = metadata
  const xScale = scaleLinear()
    .domain([0, windowCount])
  ...
  return {
    ...
    x: late
      ? plot.x + plot.width - ballRadius * 2
      : xScale(windowIndex + 0.5),
    y: plot.y + ballRadius + 2,
    vx: late ? -18 : ((index % 3) - 1) * 8,
    vy: 0,
    mass: 1,
    shape: { type: "circle", radius: ballRadius },
