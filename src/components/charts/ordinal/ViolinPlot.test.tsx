...
describe("ViolinPlot", () => {
  ...
  describe("push API", () => {
    it("ref exposes push, pushMany, getData, and clear", () => {
      const ref = React.createRef<React.ElementRef<typeof ViolinPlot>>()
      render(
        <TooltipProvider>
          <ViolinPlot ref={ref} categoryAccessor="category" valueAccessor="value" />
        </TooltipProvider>
      )
  ...
  describe("tooltip", () => {
    it("passes noop tooltip when tooltip is false", () => {
      render(
        <TooltipProvider>
          <ViolinPlot data={sampleData} tooltip={false} />
        </TooltipProvider>
      )
    ...
    it("provides default tooltipContent when tooltip is true", () => {
      render(
        <TooltipProvider>
          <ViolinPlot data={sampleData} tooltip={true} />
        </TooltipProvider>
      )
      expect(typeof lastOrdinalFrameProps.tooltipContent).toBe("function")
    })
  ...
  it("survives the loading→data transition without a hooks-count error", () => {
    ...
    try {
      ...
      rerender(
        <TooltipProvider>
          <ViolinPlot data={sampleData} categoryAccessor="category" valueAccessor="value" />
        </TooltipProvider>
      )
