...
describe("Histogram", () => {
  ...
  describe("push API", () => {
    it("ref exposes push, pushMany, getData, and clear", () => {
      const ref = React.createRef<React.ElementRef<typeof Histogram>>()
      render(
        <TooltipProvider>
          <Histogram ref={ref} categoryAccessor="category" valueAccessor="value" />
        </TooltipProvider>
      )
  ...
  describe("tooltip", () => {
    it("passes noop tooltip when tooltip is false", () => {
      render(
        <TooltipProvider>
          <Histogram data={sampleData} tooltip={false} />
        </TooltipProvider>
      )
    ...
    it("default tooltip renders category and count/range", () => {
      render(
        <TooltipProvider>
          <Histogram data={sampleData} />
        </TooltipProvider>
      ...
      // Histogram tooltips show the bin data
      const binDatum = { category: "A", count: 3, range: "10-15" }
      const { container } = render(<>{tooltipFn(binDatum)}</>)
