...
function findLeaf(
  node: NavTreeNode,
  pred: (n: NavTreeNode) => boolean
): NavTreeNode | null {
  if (node.role === "datum" && pred(node)) return node
  for (const c of node.children ?? []) {
    const f = findLeaf(c, pred)
    if (f) return f
  }
  return null
}
...
function makeHarness() {
  ...
  function Inner() {
    api.sync = useNavigationSync({
      tree,
      chartId: "c1",
      matchFields: ["month"],
      selectionName: "nav-test"
    })
...
describe("useNavigationSync — tree → canvas", () => {
  ...
  it("treats an authored hierarchy parent as a selectable semantic node", () => {
    ...
    const engineering = hierarchy.children?.[0].children?.[0]
    expect(engineering?.datum).toMatchObject({ name: "Engineering" })
    ...
    function Inner() {
      api.sync = useNavigationSync({
        tree: hierarchy,
        chartId: "hierarchy",
        selectionName: "nav-hierarchy"
      })
      api.sel = useSelection({ name: "nav-hierarchy", fields: ["name"] })
      return null
    }
    render(
      <SelectionProvider>
    ...
    act(() => api.sync.onActiveChange(engineering!))
    expect(api.sel.isActive).toBe(true)
    expect(api.sel.predicate({ name: "Engineering" })).toBe(true)
    expect(api.sel.predicate({ name: "Sales" })).toBe(false)
  })
})
...
describe("useNavigationSync — default selection name", () => {
  function nameFor(props: Parameters<typeof useNavigationSync>[0]) {
    let name = ""
    function Probe() {
      name = useNavigationSync(props).selection.name
      return null
    }
    render(
      <SelectionProvider>
  ...
  it("honors an explicit selectionName override for intentional cross-chart linking", () => {
    expect(nameFor({ tree, chartId: "c1", selectionName: "shared" })).toBe(
      "shared"
    )
  })
})
...
describe("useNavigationSync — canvas → tree", () => {
  ...
  it("maps matching raw-frame observations through its onObservation callback", () => {
    ...
    act(() =>
      api.sync.handleObservation({
        type: "click",
        datum: { month: "Mar", sales: 180 },
        x: 0,
        y: 0,
        timestamp: Date.now(),
        chartType: "line",
  ...
  it("matches GeoJSON properties by default in both tree directions", () => {
    const north = {
      type: "Feature",
    ...
    const geoTree = buildNavigationTree("ChoroplethMap", {
      areas: [north, south],
      valueAccessor: "score"
    })
    const southNode = findLeaf(geoTree, (node) =>
      node.label.startsWith("South: 3")
    )!
    const api = {} as Pick<NavigationHarnessApi, "sync" | "sel">
    function Inner() {
      api.sync = useNavigationSync({
        tree: geoTree,
        chartId: "regions",
        selectionName: "nav-regions"
      })
      api.sel = useSelection({
        name: "nav-regions",
        fields: ["type", "name", "score"]
      })
      return null
    }
    render(
    ...
    act(() => api.sync.onActiveChange(southNode))
    expect(
      api.sel.predicate({
        ...south,
        ...south.properties
    ...
    const northNode = findLeaf(geoTree, (node) =>
      node.label.startsWith("North: 8")
    )!
    expect(api.sync.activeId).toBe(northNode.id)
  })
  ...
  it("keeps the active node when an equivalent rebuilt tree retains its id", () => {
    ...
    function Inner({ t }: { t: NavTreeNode }) {
      api.sync = useNavigationSync({
        ...
        chartId: "c1",
        matchFields: ["month"],
        selectionName: "nav-rebuild-stable"
      })
      return null
    }
    const { rerender } = render(
  ...
  it("resets when a positional datum id is rebuilt for different data", () => {
    ...
    function Inner({ t }: { t: NavTreeNode }) {
      api.sync = useNavigationSync({
        ...
        chartId: "c1",
        matchFields: ["month"],
        selectionName: "nav-rebuild-replaced"
      })
      api.sel = useSelection({
        name: "nav-rebuild-replaced",
        fields: ["month"]
      })
      return null
    }
...
describe("useNavigationSync — annotation anchors", () => {
  ...
  it("focusAnnotation jumps the tree to the anchor and highlights it on the canvas", () => {
    ...
    let ok = false
    act(() => {
      ok = api.sync.focusAnnotation(annotations[0])
    }) // Feb
    expect(ok).toBe(true)
    expect(api.sync.activeId).toBe(febLeaf.id)
    expect(api.sel.predicate({ month: "Feb", sales: 250 })).toBe(true)
  ...
  it("focusAnnotation returns false (and stays put) for an annotation not anchored to a datum", () => {
    ...
    let ok = true
    act(() => {
      ok = api.sync.focusAnnotation(annotations[2])
    }) // y-threshold, no month
