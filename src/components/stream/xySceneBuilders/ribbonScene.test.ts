...
function makeBandRibbon(
  overrides: Partial<ResolvedRibbon> = {}
): ResolvedRibbon {
  return {
    kind: "band",
    getTop: (d: Datum) => d.hi,
...
function makeBoundsRibbon(
  offsetGet: (d: Datum) => number,
  overrides: Partial<ResolvedRibbon> = {}
): ResolvedRibbon {
...
describe("buildRibbonForGroup — common primitive", () => {
  it("returns null when there are fewer than 2 valid datums", () => {
    ...
    expect(
      buildRibbonForGroup(ctx, data, "default", makeBandRibbon())
    ).toBeNull()
  })
  ...
  it("uses static style override when provided", () => {
    ...
    const node = buildRibbonForGroup(
      makeCtx(),
      data,
      "default",
      makeBandRibbon({ style })
    )!
  ...
  it("interactive flag flows from ribbon to scene node", () => {
    ...
    expect(
      buildRibbonForGroup(makeCtx(), data, "g", makeBandRibbon())!.interactive
    ).toBe(false)
    expect(
      buildRibbonForGroup(
        makeCtx(),
        data,
        "g",
        makeBandRibbon({ interactive: true })
      )!.interactive
    ).toBe(true)
  })

  it("preserves group key and source row identity on the scene node", () => {
    const data = [
      { x: 1, lo: 0, hi: 10 },
      { x: 2, lo: 1, hi: 11 }
    ]
    ...
    expect(node.datum).toEqual(data)
    expect(node.datum?.[0]).toBe(data[0])
    expect(node.datum?.[1]).toBe(data[1])
  })

  it("applies the y scale to top and bottom coordinates", () => {
    const double = (v: number) => v * 2
    const doubleScale = Object.assign(double, {
      domain: () => [0, 50],
      range: () => [0, 100]
    })
    const ctx = makeCtx({
...
describe("buildRibbonForGroup — bounds (symmetric)", () => {
  ...
  it("zero or null offset collapses to topPath === bottomPath (degenerate ribbon, no gap)", () => {
    ...
    const zero = buildRibbonForGroup(
      ctx,
      data,
      "default",
      makeBoundsRibbon(() => 0)
    )!
    expect(zero.topPath).toEqual([
      [10, 30],
      [20, 50]
    ])
    expect(zero.bottomPath).toEqual([
      [10, 30],
      [20, 50]
    ])
    const nul = buildRibbonForGroup(
      ctx,
      data,
      "default",
      makeBoundsRibbon(() => null as unknown as number)
    )!
...
describe("partitionRibbons", () => {
  ...
  it("handles undefined and empty inputs", () => {
    expect(partitionRibbons(undefined)).toEqual({
      perSeries: [],
      aggregate: []
    })
...
describe("buildAggregateRibbons", () => {
  it("emits one scene node per aggregate ribbon, painted across the full dataset", () => {
    ...
    const ribbons = [
      makeBandRibbon({ perSeries: false }),
      makeBandRibbon({ perSeries: false })
    ]
