/**
 * XY scene graph → SVG elements for the browser frame and static renderer.
 * Import this leaf module at runtime so other families' serializers stay out
 * of the XY entry graph. SceneToSVG.tsx retains the shared export facade.
 */
...
export function xySceneNodeToSVG(
  ...
  return withSceneMarkCursor(element, node, `xy-cursor-${i}`)
}
...
function xySceneNodeToSVGMark(
  ...
  switch (node.type) {
    case "line": {
      ...
      const gradientId = safeSvgId(
        `${idPrefix ? `${idPrefix}-` : ""}line-${i}-stroke-gradient`
      )
    ...
    case "area": {
      ...
      const curveFactory = resolveCurveFactory(n.curve)
      const d =
        curveFactory && n.topPath.length >= 2 && n.bottomPath.length >= 2
      ...
      const areaHatch = isHatchFill(n.style.fill)
        ? hatchPatternDef(n.style.fill, areaHatchId)
        : undefined
      const areaGradientId = safeSvgId(
        `${idPrefix ? `${idPrefix}-` : ""}area-${i}-gradient`
      )
      ...
      const strokeGradientId = safeSvgId(
        `${idPrefix ? `${idPrefix}-` : ""}area-${i}-stroke-gradient`
      )
      ...
      const strokeGradient =
        hasThresholds || hasStrokeColorBands
          ? null
      ...
      const thresholdClipIds = hasStrokeColorBands
        ? n.strokeColorBands!.map((_band, bandIndex) =>
            safeSvgId(
              `${idPrefix ? `${idPrefix}-` : ""}area-${i}-stroke-band-${bandIndex}`
            )
          )
        : []
      const thresholdClipDefs = hasStrokeColorBands
        ? n.strokeColorBands!.map((band, bandIndex) => (
            <clipPath
              id={thresholdClipIds[bandIndex]}
              key={thresholdClipIds[bandIndex]}
            >
              <rect
    ...
    case "glyph": {
      ...
      return glyphNodeToSVG(
        n,
        n.x,
        n.y,
        `${idPrefix ?? ""}glyph-${n.pointId ?? i}`
      )
    }
    case "rect":
