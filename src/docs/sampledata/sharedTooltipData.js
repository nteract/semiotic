	...
	while (pointer < numPoints){
		...
		data.push({
			...
			y: pointer === 0 ?
				...
				data[pointer-1].y + (Math.floor((Math.random() * (10 - 1) + 1)) * (Math.floor(Math.random() * 2) === 1 ? 1 : -1))
		})
		pointer++;
	}
	return data;
...
["BoJack Horseman", "Todd Chavez", "Mr. Peanutbutter", "Princess Carolyn", "Diane Nguyen"]
.forEach((d, i)=>{
	lines.push({
		id: d,
