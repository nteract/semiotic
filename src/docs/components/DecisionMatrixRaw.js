...
function fetchTooltipContent(d){
  return [
    ...
    <div key={'metrics'} id="tooltip_metrics">
      <div><b>Previous Contracts: </b><span>{d['Previous Contracts']}</span></div>
      <div><b>Number Of Employees: </b><span>{d['Number of Employees']}</span></div>
    </div>
  ]
}

export default function DecisionMatrixRaw(sizeBy){
  return <div className="matrixWrapper">
    <XYFrame
        size={[750,550]}
