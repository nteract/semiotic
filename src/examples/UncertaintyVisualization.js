...
let annotations = [{
    ...
    coordinates: linesData
      ...
      .sort((a,b)=>{ return ('' + a.forecast).localeCompare(b.forecast) })
      .map((d,i)=>{
        //Ensure sorted month order (desc) and drop all non-forecasted values
        let tmpArr = d.data.sort((a,b)=>a.month-b.month).filter(x=>x.pct !== null)
...
const frameProps = {
  title: (<text textAnchor="middle">Uncertainty Visualization (Time Series)</text>),
  size: [700, 400],
  lines: linesData,
  lineDataAccessor: d => d.data,
  lineStyle:(d)=>{
    let baseStyles = {stroke:theme[1], strokeWidth:'3px'}
    if(!d.forecast){ return baseStyles}
  ...
  axes: [
    {
      orient: "left",
      tickFormat: (d,i) => {return i === 0 ? null : `${d}%`},
      label: {
        name: 'Decay (%)',
        position: {
          anchor: 'middle',
    ...
    },
    {
      orient: "bottom",
      ticks:10,
      tickFormat: (d, i) => { return i === 0 ? 'Months' : d }
    }
  ],
  tooltipContent:(d)=>{
    let projectionString = null
    if(d.parentLine.forecast){
      projectionString= <div style={{color: theme[1], fontWeight:'bold',marginBottom:'5px'}}>{`* ${d.parentLine.forecast} forecast *`}</div>
    }
    return <div 
      style={{border:`thin solid ${theme[1]}`,backgroundColor:'white', width:'100px', textAlign: 'center', padding: '10px', verticalAlign:'middle'}}>
        {projectionString}
