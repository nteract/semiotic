...
export function _mousedown(component, event) {
    component.setState({ mouseOrigin: [ event.pageX, event.pageY ], translateOrigin: component.state.translate, dragging: true });
    document.onmouseup = () => {component._mouseup(component)}
    document.onmousemove = (e) => {_mousemove(component, e)}
}
...
function _mousemove(component, event) {
    ...
    let adjustedPosition = [ event.pageX - component.state.mouseOrigin[0], event.pageY - component.state.mouseOrigin[1] ];
    let adjustedTranslate = [ (adjustedPosition[0] + component.state.translateOrigin[0]) * xAdjust, (adjustedPosition[1] + component.state.translateOrigin[1]) * yAdjust ];
    if (component.props.droppable && component.state.uiUpdate === false) {
        component.props.updateContext("dragSource", component);
        component.setState({ translate: adjustedTranslate, uiUpdate: true, dragging: true })
    }
    else {
