...
import { extractRenderEvidence } from "../src/components/server/renderEvidence"
import {
  diagnoseConfig,
  auditAccessibility,
...
async function renderChartHandler(args: { component?: string; props?: Record<string, any>; theme?: Record<string, string>; format?: string }): Promise<ToolResult> {
  ...
  if (format === "png") {
    try {
      ...
      return {
        content: [{ type: "text" as const, text: `data:image/png;base64,${base64}` }, evidenceBlock],
      }
    } catch (err: any) {
  ...
  return {
    content: [{ type: "text" as const, text: svg }, evidenceBlock],
  }
}
