...
- **Tooltip behavior needs browser evidence** (`interaction.tooltip-content-and-verification`): Interactive charts keep useful tooltips unless omission is intentional. tooltip=false or an empty callback result (null/undefined, booleans, whitespace-only strings, empty arrays/fragments) suppresses the entire tooltip; numeric zero remains visible. Suppression is independent of hover observations and selection. Verify actual content and placement in a browser; do not silently disable tooltips to avoid a defect.
