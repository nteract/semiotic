...
- **Use sub-path imports** — `semiotic/xy` (102KB gz), `semiotic/ordinal` (83KB gz), `semiotic/network` (88KB gz), `semiotic/geo` (64KB gz), `semiotic/realtime` (111KB gz), `semiotic/server` (156KB gz), `semiotic/utils` (60KB gz), `semiotic/recipes` (45KB gz), `semiotic/themes` (6KB gz), `semiotic/data` (4KB gz), `semiotic/value` (6KB gz), `semiotic/physics` (109KB gz), `semiotic/physics/matter` (1KB gz), `semiotic/physics/rapier` (1KB gz), `semiotic/ai` (363KB gz). Full `semiotic` is 282KB gz.
...
**NetworkHOPsChart** — `nodes`, `edges` with `p` or `samples`, `edgeProbabilityAccessor` ("p"), `sampleRate`, `paused`, `seed`, `anchoringStrength`, `showAggregate`, `showSampleReadout`, `edgeWidth`, `colorBy`, `size`, `frameProps`. Replays sampled or probabilistic network realizations while node positions stay anchored to the aggregate topology.
**PhysicsCustomChart** — `data`, `layout(ctx)`, `layoutConfig`, `config`, `colorBy`, `size`, `paused`, `spawnDatum`, `frameProps`. Escape hatch for bespoke physics scenes; `ctx.world` is a scratch `PhysicsPipelineStore`, and layouts return `bodies`, `colliders`/`sensors`, optional `config`, and SVG `overlays`.
