...
**ProcessSankey** — temporal sankey with a real time x-axis. `nodes`, `edges` (each with `startTime`/`endTime`), `domain` (required `[t0, t1]`), `axisTicks?`, `xExtentAccessor` (nodes can declare an explicit `[start, end]` lifetime — lane spans `min(xExtent[0], earliestEdge)` to `max(xExtent[1], latestEdge)`), `colorBy` + `colorScheme` + `showLegend` + `legendPosition` for categorical coloring, `pairing` ("value"|"temporal"), `packing` ("off"|"reuse"), `laneOrder` ("crossing-min"|"inside-out"|"crossing-min+inside-out"|"insertion"), `lifetimeMode` ("full"|"half"), `ribbonLane` ("source"|"target"|"both"), `showLaneRails`, `showQualityReadout`, `showParticles` + `particleStyle` (same ParticleStyle shape as SankeyDiagram — canvas + ParticlePool path; HOC writes bezier control points onto each ribbon edge so the shared pipeline drives them), `timeFormat`/`valueFormat`, push API via ref. Static-graph cycles are valid as long as edges move forward in time. Use when flow events have timestamps; use `SankeyDiagram` for static total-flow snapshots.
