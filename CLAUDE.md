...
- **`layoutConfig` is read only during the layout (not ingest), so changing it re-runs the layout (an in-place re-layout) WITHOUT re-ingesting topology.** Drive interaction state, styling, or animation progress through `layoutConfig` to update a custom layout cheaply per frame; changing `nodes`/`edges` (or chart `width`/`height`) is the heavier path that re-ingests. Custom overlays are read straight from the store each render (the frame's repaint re-reads them), so a recipe returning fresh JSX every layout call doesn't need any per-frame `setState`.
