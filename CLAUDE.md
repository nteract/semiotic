...
// Named preset — 15 built-in themes
<ThemeProvider theme="tufte">        {/* Tufte light */}
<ThemeProvider theme="tufte-dark">   {/* Tufte dark */}
<ThemeProvider theme="pastels">      {/* Pastels light */}
<ThemeProvider theme="bi-tool">      {/* BI Tool light */}
<ThemeProvider theme="italian">      {/* Italian Designer light */}
<ThemeProvider theme="journalist">   {/* Data Journalist light */}
<ThemeProvider theme="playful">      {/* Playful light */}
<ThemeProvider theme="high-contrast">{/* CB-safe palette, bold text */}
<ThemeProvider theme="dark">         {/* Default dark */}
...
<ThemeProvider theme={{
  colors: {
    ...
    background: "#1a1a2e", text: "#ededed", textSecondary: "#aaa",
    grid: "#333", border: "#555",
  },
...
All named presets: `light`, `dark`, `high-contrast`, `pastels`, `pastels-dark`, `bi-tool`, `bi-tool-dark`, `italian`, `italian-dark`, `tufte`, `tufte-dark`, `journalist`, `journalist-dark`, `playful`, `playful-dark`.

### Theme Serialization (`semiotic/themes`)

```jsx
import { themeToCSS, themeToTokens, TUFTE_LIGHT, resolveThemePreset } from "semiotic/themes"

// Generate CSS custom properties string (for SSR or stylesheets)
const css = themeToCSS(TUFTE_LIGHT, ".my-charts")
// → ".my-charts { --semiotic-bg: #fffff8; --semiotic-text: #111111; ... }"

// Generate design tokens (Style Dictionary / DTCG format)
const tokens = themeToTokens(TUFTE_LIGHT)
// → { semiotic: { bg: { $value: "#fffff8", $type: "color" }, ... } }

// Resolve by name
const theme = resolveThemePreset("journalist-dark") // → SemioticTheme | undefined
```
