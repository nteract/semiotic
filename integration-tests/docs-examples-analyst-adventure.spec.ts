...
async function beginInvestigation(page: Page) {
  ...
  const titleBegin = page.getByRole("button", {
  ...
  await expect(page.locator(".analyst-adventure__title-art")).toHaveText(
    "SEMIOTIC"
  )
  await expect(
    page.getByText("SEMIOTIC PRESENTS // SOFTWARE PRODUCT 1984", {
      exact: true
    })
  ).toBeVisible()
  ...
  await expect(
    page.getByText("Executive Suite · StreamXYFrame", { exact: true })
  ).toBeVisible()
}
...
async function hoverFirstExecutiveTelemetryMark(page: Page) {
  ...
  await expect(
    page.locator(".stream-xy-frame .stream-frame-tooltip")
  ).toContainText("cache age")
}
...
test.describe("Analyst Adventure authored source interactions", () => {
  test("reaches the denominator-preserving best ending using keyboard controls", async ({
    page
  }) => {
    ...
    await expect(
      page.locator("text.semiotic-chart-title", {
        hasText: "EXECUTIVE TELEMETRY // TRUST WINDOW"
      })
    ).toBeVisible()
    await expect(
    ...
    await expect(
      page.getByRole("button", { name: /Hidden Forecast Vault/i })
    ).toHaveCount(0)
    ...
    await expect(
      page.getByText(
        /dramatic destination can still be stale.*cache-age field/i
      )
    ).toBeVisible()
    ...
    await expect(
      summary.getByRole("columnheader", { name: "timestamp" })
    ).toBeVisible()
    await expect(
      summary.getByRole("columnheader", { name: "observed At" })
    ).toBeVisible()
    await expect(
      summary.getByRole("rowheader", { name: "badge-roof-0914" })
    ).toBeVisible()
    await page.keyboard.press("Escape")
    ...
    await expect(
      page.getByText("Records Catacombs · StreamOrdinalFrame", { exact: true })
    ).toBeVisible()
    ...
    await expect(
      recordsFrame
        .locator("text.semiotic-axis-tick")
        .filter({ hasText: "Sales" })
    ).toBeVisible()
    await expect(
      recordsFrame
        .locator("text.semiotic-axis-tick")
        .filter({ hasText: "Corp. Archaeology" })
    ).toBeVisible()
    await page.getByRole("button", { name: "RATE / EMPLOYEE" }).click()
    await expect(
      recordsFrame.locator("text.semiotic-chart-title")
    ).toContainText("CANCELLATIONS PER EMPLOYEE")
    await expect(
      page.getByRole("button", {
    ...
    await expect(
      page.getByRole("button", {
        name: /Corporate Archaeology.*strongest rate/i
      })
    ).toBeVisible()
    await chooseWithNumber(page, 2)
    await expectAnalyticalResolution(
      page,
      /per[- ]employee|headcount|denominator/i
    )

    await enterDestination(page, /^Corporate Map Room/)
    await expect(
      page.getByText("Corporate Map Room · StreamGeoFrame", { exact: true })
    ).toBeVisible()
    ...
    await expect(
      page.getByText("Server Cathedral · StreamNetworkFrame", { exact: true })
    ).toBeVisible()
    await expect(
      page.getByText(
        "CONSERVATION CHECK: 100 LEGITIMATE IN · 110 DISPLAYED OUT"
      )
    ).toBeVisible()
    await chooseWithNumber(page, 2)
    ...
    await expect(
      page.getByText("Boardroom · Evidence Montage", { exact: true })
    ).toBeVisible()
    await expect(
      page.getByText(
    ...
    await expect(
      page.locator(
        '.analyst-adventure__snapshot-chart[data-component="LineChart"]'
      )
    ).toBeVisible()
    await expect(
      page.locator(
        '.analyst-adventure__snapshot-chart[data-component="BarChart"]'
      )
    ).toBeVisible()
    await expect(
      page.locator(
        '.analyst-adventure__snapshot-chart[data-component="FlowMap"]'
      )
    ).toBeVisible()
    await expect(
      page.locator(
        '.analyst-adventure__snapshot-chart[data-component="SankeyDiagram"]'
      )
    ).toBeVisible()
    await chooseWithNumber(page, 1)
    await expect(
      page.getByRole("heading", {
        name: "THE BIG PRESENTATION, NOW WITH DENOMINATORS"
      })
    ).toBeVisible()
    const restoredEvidence = page.locator(".aa-ending__evidence")
    await expect(
      restoredEvidence.getByText("Eight-Minute Replay", { exact: true })
    ).toBeVisible()
    await expect(
      restoredEvidence.getByText("Denominator Key", { exact: true })
    ).toBeVisible()
    await expect(
      restoredEvidence.getByText("Origin Vector", { exact: true })
    ).toBeVisible()
    await expect(
      restoredEvidence.getByText("Lineage Break", { exact: true })
    ).toBeVisible()
    expect(maximumDepthErrors).toEqual([])
  })

  test("uses keyboard-operable annotation widgets to enter the reduced-motion vault", async ({
    page
  }) => {
    await page.emulateMedia({ reducedMotion: "reduce" })
    await beginInvestigation(page)
    await expect(page.locator(".analyst-adventure")).toHaveAttribute(
      "data-reduced-motion",
      "true"
    )
    ...
    await page.keyboard.press("Enter")
    const tunnelChat = page
      .locator("dialog.aa-chat")
      .filter({ hasText: "FACILITIES_BOT" })
    await expect(tunnelChat).toHaveAttribute("open", "")
    await expect(
      page.getByText(/This tunnel does not exist.*cannot be locked/i)
    ).toBeVisible()
    await expect(
      page.getByRole("button", { name: "Close annotation chat" })
    ).toBeFocused()
    await page.keyboard.press("Escape")
    ...
    await expect(
      page.getByText("Hidden Forecast Vault · StreamPhysicsFrame", {
        exact: true
      })
    ).toBeVisible()
    await expect(
      page.getByRole("button", { name: "SETTLED PROJECTION SHOWN" })
    ).toHaveAttribute("aria-pressed", "true")
    await expect(
      page.getByText("REDUCED MOTION: FINAL STATE", { exact: true })
    ).toBeVisible()
    await expect(
      page.getByRole("table", {
        name: "Deterministic settled projection · 30 forecast scenarios"
      })
    ).toBeVisible()
    await expect(page.getByRole("row", { name: /DEFENSIBLE 10/ })).toBeVisible()
  })

  test("reveals the Annotation Cabal on the way into the playable Forecast Vault", async ({
    page
  }) => {
    await beginInvestigation(page)
    ...
    await expect(
      page.getByRole("heading", { name: "THE ANNOTATION CABAL" })
    ).toBeVisible()
  ...
  test("keeps the analytical choice flow inside a phone viewport", async ({
    page
  }) => {
    ...
    const bounds = await page.evaluate(() => {
      ...
      if (!(root instanceof HTMLElement) || !(chart instanceof HTMLElement))
        return null
    ...
    await expectAnalyticalResolution(page, /stale|cached|observed/i)
    await expect(
      page.getByText("Eight-Minute Replay", { exact: true })
    ).toBeVisible()
