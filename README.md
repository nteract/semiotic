...
Semiotic ships 37 stable JavaScript entry points (36 subpaths plus the root). **Don't import from `"semiotic"` unless you need everything** — use the smallest sub-path that matches your charts or tooling.
...
|---|---|---|
| `semiotic/atlas` | **268 KB** | Motif Braid, Dependency Forest, and Flow Circuit readers |
| `semiotic/atlas/core` | **13 KB** | Network Atlas preparation, projections, and evidence queries |
| `semiotic/access` | **36 KB** | Chart Access Contract factory and first-wave baseline contracts |
| `semiotic/evidence` | **50 KB** | Chart Evidence Envelope, deterministic hashing, and publication gate |
| `semiotic/artifact` | **121 KB** | Renderer-independent contracts, claims, time, policy, grounding, and transfer audits |
| `semiotic/artifact/react` | **4 KB** | Accessible progressive disclosure for artifact contracts and policy evaluations |
| `semiotic/line` | **132 KB** | LineChart only — one-chart micro boundary |
| `semiotic/xy` | **162 KB** | LineChart, AreaChart, Scatterplot, Heatmap, + 8 more XY charts |
| `semiotic/ordinal` | **127 KB** | BarChart, PieChart, BoxPlot, Histogram, + 11 more categorical charts |
| `semiotic/network` | **147 KB** | ForceDirectedGraph, SankeyDiagram, ProcessSankey, Treemap, + 4 more |
| `semiotic/geo` | **105 KB** | ChoroplethMap, FlowMap, DistanceCartogram, ProportionalSymbolMap |
| `semiotic/realtime` | **159 KB** | RealtimeLineChart, RealtimeHistogram, + 4 streaming charts |
| `semiotic/realtime/core` | **158 KB** | Streaming chart types, HOCs, and buffer helpers |
| `semiotic/realtime/react` | **1 KB** | Stream status and synced push hooks |
| `semiotic/server` | **241 KB** | renderChart, renderDashboard, renderToImage, renderToAnimatedGif |
| `semiotic/server/node` | **241 KB** | renderChart, renderDashboard, renderToImage, renderToAnimatedGif |
| `semiotic/server/edge` | **248 KB** | renderChart, renderChartWithEvidence, renderToStaticSVG, renderDashboard |
| `semiotic/utils` | **98 KB** | ThemeProvider, numeric/accessibility audits, serialization — no chart components |
| `semiotic/utils/core` | **91 KB** | Pure theme helpers, numeric/accessibility audits, and serialization |
| `semiotic/utils/react` | **7 KB** | ThemeProvider, useTheme, useReducedMotion, useHighContrast, useStreamStatus |
| `semiotic/recipes` | **109 KB** | Pure layout functions (waffle, marimekko, flextree, dagre, …) |
| `semiotic/recipes/core` | **102 KB** | Pure layout functions (waffle, marimekko, flextree, dagre, …) |
...
| `semiotic/ai/core` | **134 KB** | suggestCharts, auditData, describeChart, repairChartConfig, tool adapters — no chart components |
...
| `semiotic` | **368 KB** | Full chart API and shared utilities |
...
|---|---:|---:|
| `import { MotifBraidChart } from "semiotic/atlas"` | browser | **159.3 KiB** |
| `import { prepareNetworkAtlas } from "semiotic/atlas/core"` | browser | **6.4 KiB** |
| `import { LineChart } from "semiotic"` | browser | **144.2 KiB** |
| `import { LineChart } from "semiotic/xy"` | browser | **144.6 KiB** |
| `import { LineChart } from "semiotic/line"` | browser | **144.5 KiB** |
| `import { BarChart } from "semiotic/ordinal"` | browser | **132.9 KiB** |
| `import { SankeyDiagram } from "semiotic/network"` | browser | **154.7 KiB** |
| `import { RealtimeLineChart } from "semiotic/realtime"` | browser | **145.5 KiB** |
...
| `import { renderChart } from "semiotic/server"` | node | **282.9 KiB** |
| `import { generateFrameSVGs } from "semiotic/server/edge"` | node | **146.7 KiB** |
| `import { renderToImage } from "semiotic/server/node"` | node | **283.8 KiB** |
| `import { suggestCharts } from "semiotic/ai"` | browser | **261.1 KiB** |
| `import { suggestCharts } from "semiotic/ai/core"` | browser | **46.4 KiB** |
| `import { buildArtifactContract } from "semiotic/artifact"` | browser | **19.7 KiB** |
| `import { ArtifactInspector } from "semiotic/artifact/react"` | browser | **3.8 KiB** |
| `import { createChartAccessContract } from "semiotic/access"` | browser | **33.4 KiB** |
| `import { toEvidenceEnvelope } from "semiotic/evidence"` | browser | **49.1 KiB** |
...
| `import { validateProps } from "semiotic/utils"` | browser | **10.4 KiB** |
| `import { smartTickFormat } from "semiotic/utils/core"` | browser | **8.1 KiB** |
...
**Line-boundary interpretation:** the retained named import from `semiotic/line` emits 432.0 KiB raw versus 432.0 KiB from `semiotic/xy`; gzip differs by 0.2 KiB (0.1%). Tree-shaking converges both paths on the same LineChart implementation graph. Treat `semiotic/line` as a narrower API/direct-ESM artifact boundary, not an application-bundle saving. Do not add another per-chart entry until its packed named import beats the family path by both 10 KiB gzip and 7%.
...
|----------|-------------|
| **`semiotic://artifact-contract-schema`** | Standalone JSON Schema for portable purpose, claim, evidence, timing, review, and transfer metadata. |
