...
- **AI behavior contracts** — `ai/behaviorContracts.cjs` is now the structured source for agent-visible semantic rules that schema parity cannot express: categorical color precedence, required prop combinations, push/ref behavior, ID-accessor mutation requirements, and renderChart/static-data boundaries. `semiotic-ai --doctor`, the MCP `semiotic://behavior-contracts` resource, and generated AI docs all consume the same rule metadata; `check:ai-contracts` is wired into release gates.
