...
- **`StackedAreaChart` hover-anywhere tooltip — `tooltip="multi"`** — opt-in mode that surfaces an interpolated multi-series tooltip at any x along the chart, not just within `hoverRadius` of an explicit data point. Mirrors LineChart's existing `tooltip="multi"` shape (`MultiPointTooltip()` + `tooltipMode: "multi"` forwarded to `StreamXYFrame`) so the HOC change is a 3-line branch in the frame-props assembly; the hard work happens in `StreamXYFrame`'s pointer handler. Two changes there: (1) when no scene node hits within `hoverRadius` (cursor inside the area body, far from the top edge), multi mode falls back to a cursor-anchored synthetic hit so the multi path can still run — `findAllNodesAtX` is then called with `Math.max(hoverRadius, adjustedWidth)` as the x-tolerance to disable the per-node gating that would otherwise suppress interpolation when path samples are sparse; (2) in multi mode the hover anchor uses `chartX/chartY` directly instead of `hit.x/hit.y`, eliminating the visible snap to the nearest top-path sample as the cursor sweeps across data points. The real hit's `node` still drives `hoveredNodeRef` for highlight tracking when one exists. Out of scope (deferred): crosshair line at the hovered x, click-to-lock, the same toggle on `AreaChart`. Wiring covered by 2 new unit tests in `StackedAreaChart.test.tsx`; `xy-examples/index.js` gains a `xy-stacked-area-multi-tooltip` fixture for manual + visual coverage. SSR is a no-op — tooltips are gated on `hoverPoint` (set only by pointer events), so the new prop sits dormant in static SVG output.
