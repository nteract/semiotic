...
- **Docs prerendering pipeline** — route extraction handles nested and multiline route declarations, root and nested pages get idempotent canonical/LLM alternate/JSON-LD/noscript metadata, homepage metadata is normalized consistently, generated API JSON assets are copied into the static build, and prerender functions can be imported directly for scenario tests.
