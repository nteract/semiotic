...
- **HOC shared setup unification** — `BubbleChart` and `ProportionalSymbolMap` now use `useChartSetup` for categorical color scales, legends, selection/hover/click behavior, loading/empty states, and margins while keeping chart-specific size-domain, projection, and geography logic local.
