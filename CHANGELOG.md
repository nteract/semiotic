...
- **`HoverData` unified type** — All four stream frames now construct typed `HoverData` objects instead of ad-hoc shapes. Network frames use `nodeOrEdge` field; geo frames use `properties` field. Fixed GeoFrame mismatch where tooltip and `customHoverBehavior` received different shapes.
