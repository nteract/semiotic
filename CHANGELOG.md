...
- **Drop four d3 micro-deps via inlined replacements** — audited every d3-* dependency in `package.json` and removed the four where the dependency was clearly bigger than the surface we used:
  ...
  - **`d3-time-format`** → `src/components/charts/shared/timeFormat.ts`. strftime token parser backed by `Intl.DateTimeFormat` for locale-correct month/weekday names. Handles `%Y`/`%y`/`%m`/`%d`/`%e`/`%H`/`%I`/`%M`/`%S`/`%L`/`%p`/`%b`/`%B`/`%a`/`%A`/`%j`/`%%` (every token semiotic emits internally + the realistic chart-axis subset). Pre-tokenizes the spec so per-tick formatting walks a resolved program rather than re-scanning the spec string. 13 round-trip tests cover semiotic's default `%b %d, %Y` form and each token. Dropped `%U`/`%W`/`%Z`/`%c`/`%x`/`%X`/`%f` — none appear in chart axis labels. Also drops the transitive `d3-time` (~40KB) since `d3-time-format` was its only consumer in the tree. **~120KB unpacked saved (incl. d3-time).**
