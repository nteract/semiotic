...
- **Sparse-array prop hardening across the HOC matrix** — public chart HOCs now filter `null`/non-object entries from array props (`data`, `points`, `nodes`, `edges`, `flows`, `series`, `areas`) before any iteration. CSV/loader pipelines commonly emit `[null, validRow, undefined]`-shaped input, which previously crashed the chart inside `useChartSetup`, `useColorScale`, `inferNodesFromEdges`, the choropleth geometry validator, and the StreamFrame ingestion path. A new shared identity-preserving helper at `src/components/charts/shared/sparseArray.ts#filterSparseArray` returns the original reference when nothing is dropped (preserving `useMemo` cache hits in the clean case) and is wired through three load-bearing seams: `useChartSetup` (`safeData` for hooks + the new `setup.data` return), `DataSourceAdapter.setBoundedData` (covers any frame-side ingest path), and 26 HOCs that previously did `safeData = data || []` (now `safeData = useMemo(() => filterSparseArray(data), [data])`). MultiAxisLineChart's `series` prop, the network family's `nodes`/`edges`, and ChoroplethMap's resolved `areas` get the same treatment. New regression test at `src/__tests__/scenarios/sparse-array-hardening.test.tsx` covers one HOC per data-iteration shape (single-array `data`, `series`, `nodes` + `edges`, `points`, `flows` + `nodes`, `areas`) — 14 cases, all green.
