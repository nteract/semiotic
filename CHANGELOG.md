...
- **Utils ESM barrel no longer produces a webpack warning** — `semiotic/utils`
...
- **Swimlane tracks honor rounded ends** — `trackFill` backgrounds now share `roundedTop` geometry with their lane pieces, including functional radii, in canvas and static SVG output instead of showing square track ends behind rounded value bars.
