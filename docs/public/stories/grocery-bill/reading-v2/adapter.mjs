...
var ACCOUNTING_UNITS_PER_DOLLAR = 4e3;
function fixedMoney(value, digits) {
  ...
  const displayUnits = Math.floor(
    (accountingUnits * decimalScale + ACCOUNTING_UNITS_PER_DOLLAR / 2) / ACCOUNTING_UNITS_PER_DOLLAR
  );
...
function renderReceiptHTML(receipt, snapshot, trajectorySVG = "") {
...
function trajectoryData(receipt) {
  let run = [];
...
function trajectoryChartProps(receipt) {
  ...
  return {
    ...
    description: data.length ? `One point per month, connected in calendar order from ${monthName(data[0].month)} through ${monthName(data.at(-1).month)}. Right means a more expensive basket; up means a faster annual rise. Only the latest uninterrupted run through the comparison month is connected.` : "No uninterrupted cost and annual-change pair ends in the comparison month.",
    summary: summary(receipt),
    accessibleTable: true,
    showGrid: true,
...
// docs/src/pages/examples/grocery-receipt/import.ts
import { fingerprintValue as fingerprintValue2 } from "semiotic/artifact";
function importReceiptPacket(text, snapshot) {
  if (text.length > 2e6) throw new Error("This packet exceeds the 2 MB limit.");
  const input = JSON.parse(text);
  if (!input || input.sourceFingerprint !== fingerprintValue2(snapshot).fingerprint)
    throw new Error(
      "The saved source edition is unavailable here. The current basket was retained."
    );
  const packet = verifyReceiptPacket(input);
  if (fingerprintValue2(packet.snapshot).fingerprint !== fingerprintValue2(snapshot).fingerprint)
    throw new Error("The packet source differs from the pinned edition.");
  return packet;
}

// docs/src/pages/examples/grocery-receipt/export-runtime.ts
import { renderChartWithEvidence } from "semiotic/server";
function renderBasketHTML(receipt, snapshot) {
  const props = trajectoryChartProps(receipt);
  const chart = props.data.length > 1 ? renderChartWithEvidence("ConnectedScatterplot", props) : null;
