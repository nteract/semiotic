...
A guide for creating a line chart, timeseries, difference line, and line percents using `StreamXYFrame`
...
<DocumentFrame frameProps={frameProps} type={StreamXYFrame} overrideProps={overrideProps} useExpanded />
...
This only works if you have two lines in your StreamXYFrame. Change your

- `lineType="difference"`
- `lineStyle` to return a fill color

and StreamXYFrame will automatically create cutout regions between the two lines colored by the line that
is on top.
...
Changing the `lineType="linepercent"` and StreamXYFrame will automatically sum each data point as a % of
the total for each `xAccessor` value as the y data position instead of using the raw values.

<DocumentFrame frameProps={linePercent} type={StreamXYFrame} overrideProps={overrideProps} startHidden />
...
<DocumentFrame frameProps={bumpLine} type={StreamXYFrame} overrideProps={overrideProps} startHidden />
...
To make your chart responsive, use `StreamXYFrame` with `responsiveWidth={true}`.
