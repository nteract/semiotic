...
// Denser data for the brush demo
const brushSwarmData = [
  ...Array.from({ length: 40 }, () => ({ category: "Control", value: 20 + Math.random() * 20, trial: Math.random() > 0.5 ? "A" : "B" })),
  ...Array.from({ length: 40 }, () => ({ category: "Treatment", value: 28 + Math.random() * 25, trial: Math.random() > 0.5 ? "A" : "B" })),
  ...Array.from({ length: 40 }, () => ({ category: "Placebo", value: 18 + Math.random() * 22, trial: Math.random() > 0.5 ? "A" : "B" })),
]

function SwarmBrushDemo() {
  ...
  return (
    <div style={{ background: "var(--surface-1)", borderRadius: 8, padding: 16, border: "1px solid var(--surface-3)" }}>
      <div style={{ marginBottom: 4, fontSize: 12, color: "var(--text-2)" }}>
        Drag to select a value range {brushExtent && `\u2014 ${brushExtent.r[0].toFixed(1)}\u2013${brushExtent.r[1].toFixed(1)}`}
      </div>
      <SwarmPlot {...sharedProps} brush onBrush={handleBrush} height={160} showLegend />
      <div style={{ marginTop: 8, marginBottom: 4, fontSize: 12, color: "var(--text-2)" }}>
        {brushExtent ? "Detail view" : "Detail view (brush above to zoom)"}
      </div>
      <SwarmPlot {...sharedProps} height={250} showLegend={false}
...
export default function SwarmPlotPage() {
  return (
    ...
    >
      ...
      {/* Quick Start */}
      {/* ----------------------------------------------------------------- */}
      <ChartGrounding component="SwarmPlot" />
...
<SwarmPlot data={data} brush onBrush={setExtent} height={160} />
<SwarmPlot data={data}
