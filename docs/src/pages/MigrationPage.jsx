...
function BreakingChange({ children }) {
  return (
    <div
      style={{
        ...calloutStyle,
        borderLeftColor: "var(--semiotic-danger, #ea6e6e)",
      }}
    >
      <strong
        style={{ display: "block", marginBottom: "4px", color: "var(--semiotic-danger, #ea6e6e)" }}
      >
        Breaking change
      </strong>
      {children}
...
export default function MigrationPage() {
  return (
    ...
    >
      ...
      <Tip>
        If you only have a handful of charts, start with <a href="#tldr">the TL;DR</a>. If you have
        a large legacy app, read <a href="#whats-changed">What's changed</a> first so the prop diffs
      ...
      <Tip>
        <strong style={{ display: "block", marginBottom: "4px" }}>Run the codemods.</strong>
        Most of the mechanical rewrites below — pure renames, JSX prop additions, and the sub-path
        import split — are automated by{" "}
        <a href="https://github.com/emeeks/semiotic-codemod" target="_blank" rel="noreferrer">
          <code>semiotic-codemod</code>
        </a>
        . Run the full recipe before reading the rest of this guide:
        <CodeBlock code={`npx semiotic-codemod migration-recipe ./src`} language="bash" />
        See <a href="#codemods">the codemod section</a> below for the individual transforms and
        what's left to do by hand.
      </Tip>

      <p>
        The legacy <code>XYFrame</code>, <code>OrdinalFrame</code>, and <code>NetworkFrame</code>{" "}
        names still resolve, so existing imports compile:
      </p>

      <CodeBlock code={aliasSnippet} language="jsx" />

      <p>
        New code should use the chart HOCs (<code>LineChart</code>, <code>BarChart</code>,{" "}
        <code>SankeyDiagram</code>, etc.). They wrap the Stream Frames and ship with sensible
        defaults; you'll write a fraction of the boilerplate you wrote against legacy frames.
      </p>

      <BreakingChange>
        Three frame APIs and a handful of components were removed in v3: <code>RealtimeSankey</code>
        , <code>RealtimeNetworkFrame</code>, <code>FacetController</code>, <code>ProcessViz</code>,{" "}
        <code>Mark</code>, <code>SpanOrDiv</code>, and the <code>baseMarkProps</code> prop. See{" "}
        <a href="#removed-apis">Removed APIs</a> for the migration path on each.
      </BreakingChange>
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          <tr>
            ...
            <td style={tdStyle}>
              18.1+ or 19 (peer-dep range <code>^18.1.0 || ^19.0.0</code>)
            </td>
          </tr>
          <tr>
            <td style={tdStyle}>Frames</td>
            <td style={tdStyle}>
              SVG-based <code>XYFrame</code>, <code>OrdinalFrame</code>, <code>NetworkFrame</code>
            </td>
            <td style={tdStyle}>
          ...
          <tr>
            ...
            <td style={tdStyle}>
              40+ chart HOCs (<code>LineChart</code>, <code>BarChart</code>, …)
            </td>
          </tr>
          <tr>
            <td style={tdStyle}>Streaming</td>
            <td style={tdStyle}>
              Bespoke realtime components (<code>RealtimeSankey</code>, …)
            </td>
            <td style={tdStyle}>
              Ref-based <code>push()</code> on every frame and HOC
            </td>
          </tr>
          <tr>
            ...
            <td style={tdStyle}>
              First-class — <code>"use client"</code> on every interactive component, plus a{" "}
              <code>semiotic/server</code> static-render entry point
            </td>
          </tr>
          <tr>
            <td style={tdStyle}>TypeScript</td>
            <td style={tdStyle}>
              Community <code>@types/semiotic</code>
            </td>
          ...
          <tr>
            ...
            <td style={tdStyle}>
              <code>FacetController</code>
            </td>
            <td style={tdStyle}>
              <code>LinkedCharts</code> + <code>linkedHover</code> / <code>selection</code> props
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <p>
        The most consequential change is the move from SVG marks to canvas marks. Axes, labels,
        legends, and annotations still render as SVG (so they remain inspectable, themable via CSS,
        and accessible), but data marks are painted on a canvas. This is what makes streaming, large
        datasets, and 60fps interaction feasible — but it does mean a few SVG-only patterns (CSS
        selectors targeting marks, MutationObserver, screenshot pipelines that ignored canvas) need
        adjustment.
      </p>
      ...
      <p>
        Semiotic 3 uses concurrent rendering features (transitions, deferred values) and ships with{" "}
        <code>"use client"</code> directives recognized by Next.js App Router and similar
        frameworks. The peer-dep range is <code>^18.1.0 || ^19.0.0</code>; React 16 and 17 are not
      ...
      <p>
        If you previously installed <code>@types/semiotic</code>, remove it — types now ship with
        the package.
      </p>
      ...
      <Tip>
        If you hit an error referencing <code>RealtimeSankey</code>, <code>FacetController</code>,{" "}
        <code>baseMarkProps</code>, <code>ProcessViz</code>, <code>Mark</code>, or{" "}
        <code>SpanOrDiv</code> — those were removed. Skip to{" "}
      ...
      <p>
        v3 ships 40+ chart HOC components that wrap the Stream Frames with sensible defaults,
        automatic legends, hover handling, and TypeScript generics. New code is dramatically shorter
        than the equivalent legacy Frame:
      </p>

      <p>
        <strong>Before</strong> — legacy <code>XYFrame</code> with <code>lines</code> array and{" "}
        <code>lineType</code>:
      </p>
      <CodeBlock code={lineFrameBefore} language="jsx" />

      <p>
        <strong>After</strong> — <code>LineChart</code> HOC with flat data and a string{" "}
        <code>curve</code>:
      </p>
      <CodeBlock code={lineHocAfter} language="jsx" />

      <p>
        If you'd rather stay closer to the frame API (full control, no HOC defaults), use{" "}
        <code>StreamXYFrame</code> directly:
      </p>
      <CodeBlock code={lineStreamAfter} language="jsx" />

      <p>
        The same pattern applies to network charts — <code>NetworkFrame</code> with a{" "}
        <code>networkType</code> object becomes a single-purpose HOC like{" "}
        <code>ForceDirectedGraph</code>:
      </p>
      ...
      <p>
        Each HOC accepts a <code>frameProps</code> escape hatch for advanced configuration that
        doesn't fit the prop API. See the <Link to="/charts">Charts catalog</Link> for the full list
        of HOCs and their props.
      </p>
      ...
      <p>
        ...
        ships automated transforms for the mechanical parts of the upgrade. Run the full recipe, or
        apply individual transforms one at a time. Every transform is idempotent — re-running
        produces no further changes — so it's safe to re-apply after manual edits.
      </p>
...
npx semiotic-codemod migration-recipe ./src --dry --print`}
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          <tr>
            <td style={tdStyle}>
              <code>realtime-network-frame</code>
            </td>
            <td style={tdStyle}>
              Pure rename: <code>RealtimeNetworkFrame</code> → <code>StreamNetworkFrame</code>{" "}
              across imports, JSX, and identifier references.
            </td>
          </tr>
          <tr>
            <td style={tdStyle}>
              <code>realtime-sankey</code>
            </td>
            <td style={tdStyle}>
              Renames <code>RealtimeSankey</code> → <code>StreamNetworkFrame</code> and adds{" "}
              <code>chartType="sankey"</code> to each JSX usage.
            </td>
          </tr>
          <tr>
            <td style={tdStyle}>
              <code>subpath-imports</code>
            </td>
            <td style={tdStyle}>
              Splits bare <code>from "semiotic"</code> named imports into the appropriate sub-path
              entry points (<code>semiotic/xy</code>, <code>semiotic/ordinal</code>,{" "}
              <code>semiotic/network</code>, <code>semiotic/realtime</code>,{" "}
              <code>semiotic/geo</code>, <code>semiotic/themes</code>, <code>semiotic/recipes</code>
              ). Symbols not in the manifest stay on the original <code>"semiotic"</code> import.
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <p>
        After running, format with your usual tool — codemods emit jscodeshift's default style on
        touched lines (semicolons on new statements; original style preserved on untouched ones):
      </p>
...
npx prettier --write ./src`}
      ...
      <Tip>
        Some migrations aren't automated because they're context-dependent — replacing{" "}
        <code>baseMarkProps</code> with the right per-mark style prop varies per chart, and
        rewriting <code>FacetController</code> children needs new <code>linkedHover</code> /{" "}
        <code>selection</code> props that the codemod can't infer. Those are documented below.
      </Tip>

      {/* ---------------------------------------------------------------- */}
      <h2 id="removed-apis">Removed APIs</h2>

      <p>These components and props are not exported in v3. Each has a direct replacement.</p>

      <h3 id="realtime-sankey">
        <code>RealtimeSankey</code> → <code>StreamNetworkFrame</code>
      </h3>

      <p>
        The bespoke realtime sankey was folded into <code>StreamNetworkFrame</code>'s sankey chart
        type. The push API and particle behavior are identical:
      </p>

      <CodeBlock code={sankeyDiff} language="diff" />

      <h3 id="realtime-network-frame">
        <code>RealtimeNetworkFrame</code> → <code>StreamNetworkFrame</code>
      </h3>
      ...
      <h3 id="facet-controller">
        <code>FacetController</code> → <code>LinkedCharts</code>
      </h3>

      <p>
        The cross-chart coordination model was rebuilt around explicit <code>linkedHover</code>,{" "}
        <code>linkedBrush</code>, and <code>selection</code> props that name what's being shared,
        plus a <code>LinkedCharts</code> wrapper that hosts the shared state.
      </p>

      <CodeBlock code={facetDiff} language="diff" />

      <h3 id="base-mark-props">
        <code>baseMarkProps</code> (removed)
      </h3>

      <p>
        Two replacements depending on what you used <code>baseMarkProps</code> for:
      </p>
      <ul>
        <li>
          For <strong>per-mark styling</strong> (fill, stroke, opacity), use the per-mark style
          props — <code>lineStyle</code>, <code>pointStyle</code>, <code>pieceStyle</code>, etc.
          Each accepts either a static style object or a function returning one.
        </li>
        <li>
          For <strong>transition / animation timing</strong>, use the <code>animate</code> prop on
          the HOC. v3 marks render to canvas, so CSS transitions on style objects do not animate
          mark updates — <code>animate</code> wires the duration/easing into the canvas transition
          pipeline, which is what produces the actual smooth interpolation.
        </li>
      </ul>
      ...
      <p>
        <code>animate</code> accepts <code>true</code>, a number of milliseconds, or{" "}
        <code>{`{ duration, easing, intro }`}</code>. The{" "}
        <Link to="/api/charts">chart API reference</Link> lists per-chart defaults.
      </p>

      <h3 id="process-viz">
        <code>ProcessViz</code>, <code>Mark</code>, <code>SpanOrDiv</code> (removed)
      </h3>
      ...
      <p>
        Data marks are painted on a canvas in v3. Existing code that reaches into the DOM to inspect
        or manipulate marks (CSS selectors, <code>MutationObserver</code>, screenshot tools that
        walk the SVG tree) won't see marks anymore. Style overrides should move to the per-mark
        style props (<code>lineStyle</code>, <code>pieceStyle</code>, <code>pointStyle</code>, etc.)
        or to theme-level CSS variables.
      </p>

      <Tip>
        Axes, labels, legends, annotations, and tooltips remain SVG/HTML in v3. Theming and
        accessibility tools targeting those layers continue to work without changes.
      </Tip>

      <h3 id="network-style-callbacks">
        Network style callbacks see <code>RealtimeNode</code>
      </h3>

      <p>
        On <code>StreamNetworkFrame</code> (and the network HOCs), <code>nodeStyle</code> and{" "}
        <code>edgeStyle</code> functions receive a <code>RealtimeNode</code>/
        <code>RealtimeEdge</code> wrapper. Your original data lives on <code>d.data</code> instead
        of directly on <code>d</code>.
      </p>
      ...
      <p>
        ...
        <code>ref.current.push()</code> to ingest live data; the frame handles windowing, decay,
        transitions, and re-paint. Omit the <code>data</code> prop entirely when streaming — passing
        an empty array clears the chart on every render.
      </p>

      <CodeBlock code={streamingSnippet} language="jsx" />

      <p>
        See the <Link to="/features/push-api">Push API</Link> page for the full ref shape (
        <code>push</code>, <code>pushMany</code>, <code>remove</code>, <code>update</code>,{" "}
        <code>clear</code>, <code>getData</code>, <code>getScales</code>) and the streaming
        encodings (<code>decay</code>, <code>pulse</code>, <code>staleness</code>,{" "}
        <code>transition</code>) you can compose on top.
      </p>
      ...
      <p>
        v3 ships its own type definitions. Remove any <code>@types/semiotic</code> from your
        dependencies. Every chart HOC accepts a generic for the row type so accessor strings are
      ...
      <p>
        For static SVG output that doesn't require a browser at all (email, OG images, PDFs,
        dashboards rendered into static sites), use the <code>semiotic/server</code> entry point:
      </p>

      <CodeBlock code={staticSvgSnippet} language="ts" />

      <p>
        See <Link to="/using-ssr">Server-side rendering</Link> for the full SSR story.
      </p>
      ...
      <p>
        v2 was a series of release candidates (up to <code>2.0.0-rc.12</code>) that began the
        internal refactoring to functional components and TypeScript. It never promoted to a stable
        release. v3 completes that work and adds the chart HOCs, the stream-first Frames, SSR,
        sub-path imports, and built-in types.
      </p>
      ...
      <p>
        Open an issue at{" "}
        <a href="https://github.com/nteract/semiotic/issues" target="_blank" rel="noreferrer">
          github.com/nteract/semiotic/issues
        </a>
        . Include the legacy snippet you're migrating from and what the v3 equivalent should
        produce.
      </p>
    </PageLayout>
  )
