...
// capacity usage: a constant "Fast scaling" base (10) plus a
...
function buildPlaygroundRules(state, field) {
  ...
  const rules = [
    { when: cond(state.op, state.threshold), style: styleFor(state.color, state.fillMode) },
  ]
  if (state.second) {
    rules.push({
      when: cond(state.op, state.threshold2),
      style: styleFor(state.color2, state.fillMode2),
    })
  }
  return rules
}
      ...
      <div style={ctrlRow}>
        ...
        >
          <input
            ...
            onChange={(e) => set({ second: e.target.checked })}
          />
          Layer a 2nd rule
        </label>
      </div>
      {state.second && (
        <div
          style={{
            ...
            marginBottom: 0,
            paddingTop: 8,
            borderTop: "1px dashed var(--semiotic-border, #ddd)",
          }}
        >
          <span style={{ fontSize: 12, color: "var(--text-secondary, #667)" }}>
            2nd rule (same operator — later rule wins on overlap):
          </span>
          <label style={ctrlLabel}>
          ...
          <label style={ctrlLabel}>
            ...
            <select
              style={ctrlField}
              value={state.fillMode2}
              onChange={(e) => set({ fillMode2: e.target.value })}
            >
              <option value="hatch">hatch</option>
              <option value="solid">solid</option>
            </select>
...
const NETWORK_EDGES = [
  ...
  { source: "api", target: "db-users" },
  { source: "api", target: "queue" },
  { source: "queue", target: "worker-1" },
  { source: "queue", target: "worker-2" },
  { source: "worker-1", target: "db-orders" },
  { source: "worker-2", target: "db-orders" },
  { source: "api", target: "search" },
  { source: "search", target: "cache" },
  { source: "auth", target: "db-users" },
]
...
export default function StyleRulesPage() {
  return (
    ...
    >
      <p>
        ...
        <strong>threshold-based style rules</strong> that recolor or re-texture marks by value,{" "}
        <strong>declarative hatch fills</strong> that read identically on canvas and in exported
        SVG, and
        <strong> annotation label backgrounds</strong> (a stroke halo or a semitransparent box) that
        keep reference-line labels legible over dense marks. They were built to reproduce dashboards
        like the capacity chart below — but each works on its own.
      </p>

      <h2 id="flagship">Putting it together: a capacity chart</h2>
      <p>
        A stacked bar with a solid <em>Fast scaling</em> base and a yellow-hatched{" "}
        <em>Fixed-rate</em> burst on top. Wherever a bar breaks the <code>Max · 15</code> ceiling,
        the portion above the line is split into its own segment and hatched <strong>red</strong> —
        the overage reads as a distinct, alarming texture. Plus two reference lines: the red{" "}
        <code>Max · 15</code> ceiling with the default halo label, and a green{" "}
        <code>Fast-scaling · 10</code> line whose label sits in a semitransparent box so it stays
        readable where it crosses the bars.
      </p>
      <LiveExample
        type={StackedBarChart}
        title="hatch fill + over-max highlight + threshold lines"
        startHidden={false}
        frameProps={{
          data: usageData,
          categoryAccessor: "t",
          ...
          styleRules: [
            {
              ...
              style: {
                fill: { type: "hatch", background: "#f7d774", stroke: "#e0a92a", spacing: 6 },
              },
            },
            {
              ...
              style: {
                fill: { type: "hatch", background: "#f8b4b4", stroke: "#d7263d", spacing: 6 },
              },
            },
          ],
          annotations: [
      ...
      />
      <p>
        A stacked-bar rule styles a whole <em>segment</em>, so "the part over the ceiling" has to be
        its own segment — the sample data splits each fixed-rate burst at 15 into a{" "}
        <code>Fixed-rate</code> piece (up to the line) and an <code>Over max</code> piece (above
        it), and two rules hatch them yellow and red respectively.
      </p>

      <h2 id="style-rules">Threshold style rules</h2>
      <p>
        <code>styleRules</code> on <code>BarChart</code>, <code>StackedBarChart</code>, and{" "}
        <code>GroupedBarChart</code> is an ordered list of <code>{"{ when, style }"}</code> pairs.
        Every rule whose <code>when</code> condition matches a bar contributes its{" "}
        <code>style</code>, and the styles are merged in list order — so for any single property,{" "}
        <strong>the last applicable rule wins</strong>. It is the CSS-cascade model: send as many
        rules as you like; the last one that applies to a property is the one you see. Bars that
        match no rule keep the chart's resolved base color.
      </p>
      <LiveExample
        type={BarChart}
      ...
      />
      <p>
        A bar of value 12 matches the first rule (≥ 10) → amber. A bar of value 17 matches both
        rules; the second (≥ 15) wins → red. Bars below 10 match nothing and keep the base{" "}
        <code>color</code>. Because merging is per-property, an early rule can set{" "}
        <code>stroke</code> and a later rule can override just the <code>fill</code> without
        touching the stroke.
      </p>

      <h3 id="when">
        The <code>when</code> condition
      </h3>
      <p>
        <code>when</code> accepts three forms. Omit it (or pass <code>true</code>) for an
        unconditional base layer; pass <code>false</code> to disable a rule without deleting it.
      </p>
      <table className="reference-table">
        ...
        <tbody>
          <tr>
            ...
            <td>
              <code>{"{ gte: 10, lt: 15 }"}</code>
            </td>
            <td>
              Numeric/categorical matching with no code. Compares the bar's value unless{" "}
              <code>field</code> is set.
            </td>
          </tr>
          <tr>
            <td>Predicate function</td>
            <td>
              <code>{"(d, ctx) => ctx.category === 'burst'"}</code>
            </td>
            <td>
              Full control. <code>ctx</code> is <code>{"{ value, category, index }"}</code>;{" "}
              <code>d</code> is the raw datum.
            </td>
          </tr>
          <tr>
            <td>Boolean</td>
      ...
      <CodeBlock
        language="ts"
        code={`interface StyleRuleThreshold {
  ...
  in?: Array<number | string> // value is one of these
      ...
      <p>
        ...
        <code>CanvasPattern</code> when drawn to canvas in the browser and to an SVG{" "}
        <code>{"<pattern>"}</code> when serialized on the server. One declaration, both backends, so
        a hatched bar looks the same live and in an exported PNG/SVG.
      </p>
      <CodeBlock
        language="ts"
        code={`interface HatchFill {
  type: "hatch"
  ...
  lineOpacity?: number  // SVG line opacity (default 1)
      ...
      />
      <p>
        The same descriptor works as a <em>region</em> fill on <code>band</code> and{" "}
        <code>x-band</code> annotations (<code>fill: {"{ type: 'hatch', … }"}</code>), so an
        uncertainty or "projected" region can be hatched rather than flat-shaded. For a hand-built{" "}
        <code>CanvasPattern</code> (canvas only), <code>createHatchPattern()</code> is still
        available — see <Link to="/theming/styling">Styling</Link>.
      </p>

      <h2 id="label-backgrounds">Annotation label backgrounds</h2>
      <p>
        Region-bounding annotations — <code>y-threshold</code>, <code>x-threshold</code>,{" "}
        <code>band</code>, <code>x-band</code>, <code>enclose</code>, <code>rect-enclose</code>, and{" "}
        <code>category-highlight</code> — accept a <code>labelBackground</code> that puts a
        legibility backdrop behind the label text. It works on every frame (XY, ordinal, network,
        geo) and in server SVG, because all of them share one label renderer.
      </p>
      <table className="reference-table">
        <thead>
          <tr>
            <th>
              <code>labelBackground</code>
            </th>
            <th>Result</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>
              <code>"halo"</code> / <code>true</code>
            </td>
            <td>
              Stroke halo (a thick outline in the plot background color) — the default for threshold
              and band labels.
            </td>
          </tr>
          <tr>
            ...
            <td>A filled, semitransparent rounded panel behind the text.</td>
          </tr>
          <tr>
            <td>
              <code>"none"</code> / <code>false</code>
            </td>
            <td>Plain text, no backdrop.</td>
          </tr>
          <tr>
            <td>
              <code>{"{ type, fill, opacity, padding, radius, stroke, haloWidth }"}</code>
            </td>
            <td>Fine-grained control over either treatment.</td>
          </tr>
        </tbody>
      </table>
      <LiveExample
      ...
      <p>
        Scatterplot rules see the raw datum, so an x <em>and</em> y threshold both resolve. Here
        points in the top-right quadrant are flagged and the far outliers hatched.
      </p>
      <LiveExample
        type={Scatterplot}
        ...
        frameProps={{
          ...
          styleRules: [
            ...
            {
              when: (d) => d.x >= 85 && d.y >= 85,
              style: { fill: { type: "hatch", background: "#d7263d", stroke: "#fff", spacing: 5 } },
            },
          ],
        }}
      ...
      <p>
        Rules see the raw node; <code>ctx.value</code> is the node's <code>weight</code> here. Drag
        the threshold, switch solid/hatch, and toggle a second rule to watch{" "}
        <em>last-applicable-rule-wins</em> repaint the overlapping nodes. The layout re-settles on
        data change but a rule change only restyles — a good way to confirm nothing breaks.
      </p>
      <NetworkStyleRulesDemo />

      <h3 id="geo-playground">Geo features — interactive</h3>
      <p>
        The same controls over a <code>ChoroplethMap</code> (each country is seeded with a 0–100
        value). Rules layer over the sequential base fill, so unmatched countries keep their scale
        color while matched ones flip to your rule's solid color or hatch.
      </p>
      <GeoStyleRulesDemo />

      <h3 id="physics-family">Physics particles</h3>
      <p>
        Physics particles use the identical API — <code>ctx.value</code> is the body's value. (Shown
        as code to keep the <code>semiotic/physics</code> kernel out of this page's bundle.)
      </p>
      <CodeBlock
        language="tsx"
        code={`// Physics — recolor particles over a threshold (ctx.value = the body's value)
<GaltonBoardChart
...
/>`}
      ...
      <p>
        <code>styleRules</code>, hatch fills, and <code>labelBackground</code> all work through{" "}
        <code>renderChart()</code> and the MCP <code>renderChart</code> tool — the props are plain
        JSON, so an agent can emit them directly. The capacity chart above, rendered server-side:
      </p>
      <CodeBlock
        language="ts"
...
const svg = renderChart("StackedBarChart", {
  ...
  annotations: [
    ...
    { type: "y-threshold", value: 10, label: "Fast-scaling · 10", color: "#2f7d32",
      ...
      <p className="callout">
        ...
        <code>{"{ field, eq }"}</code>) over a predicate function when the config must be serialized
        as JSON — predicate functions can't cross a JSON boundary.
      </p>
  ...
  > frameProps.pieceStyle  (imperative escape hatch)
    > styleRules           (declarative, last-applicable rule per property)
      ...
      <ul>
        <li>
          <Link to="/charts/bar-chart">Bar Chart</Link> — the base chart and its other styling
          props.
        </li>
        <li>
          <Link to="/theming/styling">Styling</Link> — primitive styling, gradients, and the canvas{" "}
          <code>createHatchPattern()</code> helper.
        </li>
        <li>
          <Link to="/annotations/overview">Annotations</Link> — the full annotation type reference.
        </li>
      </ul>
    </PageLayout>
  )
