...
rendering. StreamXYFrame supports line charts, bar charts, scatter/swarm plots, and waterfall
charts — all animated and continuously updating as new data arrives.

Unlike other Semiotic frames, StreamXYFrame uses an imperative API via React refs: create a ref
...
```jsx
<StreamXYFrame chartType="line" />
<StreamXYFrame chartType="bar" />
<StreamXYFrame chartType="swarm" />
<StreamXYFrame chartType="waterfall" />
```
...
```jsx
<StreamXYFrame margin={{ top: 20, right: 20, bottom: 30, left: 40 }} />
```
...
<StreamXYFrame arrowOfTime="right" />

<!-- New data appears on the left, old data scrolls right -->
<StreamXYFrame arrowOfTime="left" />
```
...
```jsx
<StreamXYFrame className="my-realtime-chart" />
```
...
```jsx
<!-- String option -->
<StreamXYFrame timeAccessor="timestamp" />

<!-- Function option -->
<StreamXYFrame timeAccessor={d => d.ts / 1000} />
```
...
```jsx
<!-- String option -->
<StreamXYFrame valueAccessor="price" />

<!-- Function option -->
<StreamXYFrame valueAccessor={d => d.high - d.low} />
```
...
```jsx
<StreamXYFrame windowMode="sliding" windowSize={300} />
<StreamXYFrame windowMode="growing" />
```
...
```jsx
<StreamXYFrame timeExtent={[0, 1000]} />
```
...
<StreamXYFrame valueExtent={[0, 100]} />
```
...
```jsx
<StreamXYFrame extentPadding={0.2} />
```
...
```jsx
<StreamXYFrame background="#f8f9fa" />
```
...
```jsx
<StreamXYFrame showAxes={true} />
<StreamXYFrame showAxes={false} />
```
...
```jsx
<StreamXYFrame tickFormatTime={t => new Date(t).toLocaleTimeString()} />
```
...
```jsx
<StreamXYFrame tickFormatValue={v => `$${v.toFixed(2)}`} />
```
...
```jsx
<StreamXYFrame chartType="bar" binSize={20} />
```
...
```jsx
<!-- Simple boolean -->
<StreamXYFrame hoverAnnotation={true} />
...
StreamXYFrame exposes an imperative API via React refs. This is the recommended way to stream
...
<StreamXYFrame ref={frameRef} chartType="line" />
```
