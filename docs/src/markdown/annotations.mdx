...
- `x`: creates an xy-threshold annotation along the x axis, you must pass a prop that matches the `xAccessor` used for the `StreamXYFrame`
- `y`: creates an xy-threshold annotation along the y axis, you must pass a prop that matches the `yAccessor` used for the `StreamXYFrame`
...
- `or`: a circle annotation with a `"label"` (like "xy" for StreamXYFrame)
...
- StreamNetworkFrame: `d, i, networkFrameProps, networkFrameState, nodes, edges, voronoiHover, screenCoordinates, adjustedPosition, adjustedSize, annotationLayer`
- StreamXYFrame: `d, i, xyFrameProps, xyFrameState, xScale, yScale, xAccessor, yAccessor, summaries, points, lines, voronoiHover, screenCoordinates, adjustedPosition, adjustedSize, annotationLayer`
