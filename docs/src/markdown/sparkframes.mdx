...
`<StreamNetworkFrame />` is a wrapper around StreamNetworkFrame. Some of the built-in defaults of the frame are different from the usual StreamNetworkFrame:
...
`<StreamXYFrame />` is a wrapper around StreamXYFrame. It also has a slightly different default for the `axes` prop:
...
`<StreamOrdinalFrame />` is a wrapper around StreamOrdinalFrame. Its only difference in defaults is the margin settings shared by all spark frames.
